...
サイドバーの構成は上から エンジン制御・監視 / 設定 / 書き込み先 /
書き込みルール / 書き込み監査ログ / ユーザー管理 / 監査ログ です
（ユーザー管理と監査ログは管理者のみ表示）。
...
| --------------- | ----------------------------------------------------------------------- |
| 閲覧者 (viewer) | 各画面の閲覧のみ                                                        |
| 編集者 (editor) | + 書き込み先/ルールの作成・編集・削除、ドライラン切替                   |
...
- 上段の「新規作成」フォームで名前・PLC 接続 ID・アドレス（例: `D200`）・
...
> 注意: 「PLC 接続 ID」は数値の直接入力です。PLC 接続の一覧画面が
> まだ無いため（[§6](#6-現状の制限)）、ID はデータベース登録時の値を
> 指定します。存在しない ID はサーバー側の検証でエラーになります。
...
- **条件（すべて AND で判定されます）**: 「+ 条件を追加」で行を増やせます。
  各行はソースタグ ID・演算子（=, ≠, >, ≥, <, ≤, between, bit_is）・
...
> ソースタグ ID も現状は数値の直接入力です（[§6](#6-現状の制限)）。
...
1. **登録**: PLC 接続・ソースタグを登録し（現状は [§6](#6-現状の制限) の
   方法で）、「書き込み先」→「書き込みルール」の順に画面から登録します。
...
- **PLC 接続・ソースタグの管理画面が未実装です。** 書き込み先やルールが
  参照する PLC 接続 ID・ソースタグ ID は画面からは登録できず、現状は
  データベース（SQLite）への直接登録が必要で、各フォームでは ID を数値で
  直接入力します。管理画面は将来のマイルストーンで追加予定です。
