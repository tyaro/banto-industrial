...
//! `cargo test -p relay-wright-core --lib` run left 17 directories behind -
//! matching this module's every fixture that ever created one.
//!
...
//! unique naming (`into_path()` hands the directory over without deleting
//! it), but takes over cleanup itself with a retry. This requires every
//! test using it to run on a multi-thread tokio runtime with >= 2 workers
