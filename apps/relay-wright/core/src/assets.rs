...
//!
//! The SvelteKit static build (`apps/relay-wright/build`) is gitignored
...
//!
//! To build with the real frontend: `pnpm --filter relay-wright build`
//! first (produces `apps/relay-wright/build`), then
//! `cargo build -p relay-wright-core --features embed-ui`.
...
mod embedded {
    ...
    /// crate (`apps/relay-wright/core`), so `../build` is
    /// `apps/relay-wright/build`.
    #[derive(RustEmbed)]
    #[folder = "../build"]
    pub struct Assets;
}
...
const PLACEHOLDER_HTML: &str = concat!(
    ...
    "<title>RelayWright</title></head><body>",
