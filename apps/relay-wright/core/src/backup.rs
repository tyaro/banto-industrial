...
//! called from `src-tauri`'s `run()` and `bin/relay-wright-serve.rs`'s `main`
//! *before* `relay_wright_core::db::init_db`.
//!
//! ## Directory layout
//!
//! Given the DB file at `{dir}/relay-wright.sqlite3`:
...
/// Tables a file must have to be accepted as a restorable RelayWright
...
/// cheap sanity check that this is a RelayWright DB at all (not, say, a
...
/// before the swap - the caller (spec: `run()`/`relay-wright-serve`'s `main`)
...
/// Validate that `path` is an intact, restorable RelayWright database (spec M17):
...
async fn run_validation_checks(conn: &mut SqliteConnection) -> Result<(), BantoError> {
    ...
    for table in REQUIRED_TABLES {
        ...
        if exists.is_none() {
            return Err(validation_err(format!(
                "必須テーブル '{table}' が見つかりません。RelayWright のデータベースファイルではない可能性があります"
            )));
        }
    }
...
impl BackupService {
    ...
    /// `"db.sqlite3"`, as `bin/relay-wright-serve.rs`'s `BANTO_DB` default can be) -
    ...
    /// yet at the call site (`src-tauri`'s `run()`/`bin/relay-wright-serve.rs`'s
    /// `main`, both before `relay_wright_core::db::init_db`).
    ///
...
mod tests {
    ...
    /// `src-tauri`/`bin/relay-wright-serve.rs` is always a real on-disk file.
