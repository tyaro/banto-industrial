...
//! `relay_wright_core::assets::FrontendAssets`, which is applied
//! internally - this file does not need its own `#[cfg(feature = ...)]`.
//!
//! ```text
//! pnpm --filter relay-wright build   # produces apps/relay-wright/build
//! cargo run -p relay-wright-core --bin relay-wright-serve --features embed-ui
//! ```
//!
...
async fn main() {
    ...
    let applied_restore = match BackupService::apply_pending_restore_at_startup(&db_path_buf).await
    {
        Ok(applied) => applied,
        Err(err) => {
            eprintln!("relay-wright-serve: 起動時のリストア適用に失敗しました: {err}");
            None
        }
    };
    ...
    if let Some(applied) = applied_restore {
        audit
            ...
            .await;
        println!(
            "relay-wright-serve: 起動時にリストアを適用しました（適用前の自動バックアップ: {}）",
    ...
    println!("relay-wright-serve: DB at {db_path}");
    println!("relay-wright-serve: listening at:");
    for url in lan_urls(server.local_addr().port()) {
        println!("  {url}");
    }
    if allow_setup {
        println!("relay-wright-serve: first-run setup is ENABLED (BANTO_ALLOW_SETUP=1) - POST /api/auth/setup will create the first account");
    } else {
        println!(
            "relay-wright-serve: first-run setup is DISABLED - set BANTO_ALLOW_SETUP=1 to allow POST /api/auth/setup"
        );
    }
    println!("relay-wright-serve: press Ctrl-C to stop");
