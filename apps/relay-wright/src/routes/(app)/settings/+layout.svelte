<script lang="ts">
	/**
	 * 設定画面のシェル（#359 relay-wright 分）。banto-hub（#371）・
	 * chronogazer（#372）の settings/+layout.svelte にならい、カテゴリナビ +
	 * 各カテゴリのルート（`{@render children()}`）をここに集約する。
	 *
	 * relay-wright には template の `PageHeader` コンポーネントが無く、
	 * ページ見出しは既存どおり `Header.svelte` の `<h1>`（`pageTitle()`、
	 * `navigation.ts` 参照）が担っている（他の画面もページ内に別途タイトルを
	 * 描画していない）ため、ここでは独自の見出しを新設せずカテゴリナビだけを
	 * 追加する。
	 *
	 * `data.categories` は `+layout.ts` が現在のセッションで見える subset に
	 * 絞り込み済みなので、ナビには常にアクセス可能なカテゴリしか出ない
	 * （分割前の `{#if}` ガードと同じ「見せない」方針を実ルートに適用した
	 * もの）。
	 *
	 * `authSettingsStore`（ログイン不要モード＋自動ログインの共有 state）の
	 * 初回ロードをここへ集約する（`tauri` 限定という既存条件は元
	 * `+page.svelte` の effect と同じまま維持 - `authSettingsStore.svelte.ts`
	 * の doc comment参照）。継続ポーリングは元々不要（元 `+page.svelte` も
	 * 1回読むだけだった）なので、ここでは1回だけ `load()` する。失敗時は
	 * `authSettingsStore.error` に記録する（元 `+page.svelte` の `authError`
	 * と同じ見た目を `SecuritySection.svelte` 側で保つため）。
	 */
	...
	$effect(() => {
		if (!tauri) return;
		void authSettingsStore.load().catch((err) => {
...
<style>
	...
	/* 1024px 以上: 左レール sticky、右にコンテンツ。sticky の offset は
	   Header.svelte の高さトークンに合わせる - レールは固定ヘッダーの下を
	   クリアしつつ本文だけがスクロールする（Sidebar.svelte の `aside` と
	   同じ考え方）。 */
