<script lang="ts">
	/**
	 * 外観カテゴリ（テーマ/プリセット/ウィンドウ効果）（#359 relay-wright
	 * 分）。元 `+page.svelte` の該当セクションから markup・state・関数を
	 * 無改変で移した。常に表示（テーマ/プリセット自体にガード無し。
	 * ウィンドウ効果は `tauri && isAdmin && vibrancyStatus?.supported` の
	 * ときだけ内部で描画）。
	 */
