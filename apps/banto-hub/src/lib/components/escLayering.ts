 ...
 *    出た瞬間（この辺りの部品はフォーカストラップを持たない）にその層は閉じられ
 ...
 *
 * 4. **モーダル層（`Drawer`/`Modal`）はフォーカストラップで裏の起動経路を断つ**
 *    （`focusTrap.ts`）。フォーカスがパネルの外へ出られると、オーバーレイの裏に
 *    ある起動ボタンへ Tab で届いて別の層を開けてしまい、重なりの前提（1本の
 *    積み重ね）が崩れる。
 * 5. **非モーダル層（サイドバー・退避ツリー）は、下位を開くときに上位を先に畳む**
