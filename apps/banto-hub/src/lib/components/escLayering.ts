 ...
 * **呼び出し側の契約（`focusFallback` など）を足したときは、共有部品の呼び出し元を
 * 全部見直すこと**（#381 レビュー対応20回目）: `Drawer`/`Modal` は複数の画面から
 * 使われていて、渡し忘れた画面だけ「閉じたらフォーカスが `<body>` に落ちる」形で
 * 残る（実際に `/sink` の Sink グループ Drawer が取り残されていた）。
 * `grep -rn "<Drawer\|<Modal" apps/banto-hub/src apps/banto-hub/src` で列挙して、**削除や一覧
 * 更新で開いた元の要素が消えうる経路**を持つものすべてに渡す。
 *
