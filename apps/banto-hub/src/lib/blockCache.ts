/**
 * **ブロック単位の遅延取得キャッシュ**（汎用部）。chronogazer の
 * `apps/chronogazer/src/lib/blockCache.ts`（#409 / #410）の**複製**（#428）。
 *
 * **このコメント以外は chronogazer 側と 1 文字も違わない**ことを
 * `blockCache.sync.test.ts` が確かめる - 直すときは両方を同じに直すこと
 * （アプリ間で共有するパッケージがまだ無いため、複製と一致テストで保つ。
 * 共有化の選択肢は #428 の PR 本文）。設計の説明（世代とスナップショット
 * 境界・どのブロックを取るか・失敗をブロック単位で持つ・自動では再試行
 * しない）は chronogazer 側の同じ位置のコメントにある。
 *
 * banto-hub で使うのは `/audit-log`（`routes/(app)/audit-log/auditBlocks.ts`）
 * だけ。`audit_log.id` が `AUTOINCREMENT`（`core/src/db.rs` の
 * `apply_app_schema`）であることは確かめてある。
 */
