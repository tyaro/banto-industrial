/**
 * `auditBlocks.ts` のユニットテスト（#428）。chronogazer の同名テスト（#410）と
 * 同じ中身で、純関数の表テストと、偽の `audit_log` 相手に `AuditBlockLoader` を
 * 回す回帰テスト。
 *
 * 回帰テストは、banto-hub の修正前の `AuditLogWindow`（`+page.svelte` に
 * 埋め込まれていたブロック読み込み）で**実際に起きた**ことを確かめた
 * シナリオを使う（再現の結果は PR #428 の本文）:
 *
