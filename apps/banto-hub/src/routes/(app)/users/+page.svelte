<script lang="ts">
	/**
	 * ユーザー管理画面（spec M10 RBAC）。`admin` のみ到達（+page.ts が
	 * 非adminをリダイレクト）。chronogazer の同名ファイルから複製し、
	 * デモモード分岐（`isUsersAdminAvailable()`/`DEMO_MODE_MESSAGE`）を
	 * 削除した - banto-hub にはデモモードが存在しない（usersAdmin.ts 参照）。
	 *
	 * 一覧はBantoGrid（クライアントモード、全件を一度に取得）。行クリックで
	 * 選択→下に詳細/編集/リセット/削除パネルを表示する構成は無改変。
	 */
