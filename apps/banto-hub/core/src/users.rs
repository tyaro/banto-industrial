//! Local credential store (docs/tag-server-design.md §5.6 "管理 UI: bearer
//! token セッション"): a `users` table in banto-hub's own SQLite DB, with
//! argon2id password hashes. Copied near-verbatim from
//! `apps/chronogazer/core/src/users.rs` - RBAC (admin/editor/viewer) and the
//! account model have no hub-specific twist in T0.
//!
//! Design note (keyring for credentials): keyring is a
...
impl Role {
    ...
    /// predicate every role guard (`crate::rest`'s `require_role_at_least`/
    /// `require_editor` middleware - banto-hub has no Tauri command surface,
    /// REST is its only transport) is built on.
...
impl FromStr for Role {
    ...
    /// `role` TEXT column back out of SQLite and (in `chronogazer-core::rest`)
...
/// Identity of a verified/created user. `banto_server::Identity` (the wire
/// type a bearer session actually carries) has only `id`/`name`/`role`; this
/// carries the full row `crate::rest`'s REST handlers need (e.g. `username`,
/// to look the account back up for `change_password`).
...
/// the admin UI needs, deliberately NOT `password_hash`. Unlike
/// [`UserIdentity`] this derives `Serialize` since it is returned directly
/// over the wire (REST JSON body) rather than only used internally.
...
impl UsersService {
    ...
    /// field name `newPassword` so `crate::rest` can map the error straight
    /// onto the change-password form's second input).
    ...
    /// Delete account `id`. Refuses two cases with a `BantoError`: deleting
    /// the last `admin`, and an account deleting itself (`acting_user_id`,
    /// the caller's own numeric row id, is resolved by `crate::rest` before
    /// calling this - see [`UsersService::get_by_username`], which only has
    /// the caller's username from the bearer session token).
