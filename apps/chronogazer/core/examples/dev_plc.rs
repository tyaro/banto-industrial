...
//! ## なぜ製品の中ではなく別プロセスなのか
//!
//! 製品の接続単位シミュレーション（`ctx.simulation`）は値が **tstore に
//! 記録されない**（`crates/banto-collect/src/task.rs` の
//! `if ctx.simulation { return; }` 付近）。R1-C の完了条件「設定 → 収集開始
//! → データファイル生成 → イベント記録まで一巡」はデータファイル生成まで
//! 含むため、これでは確かめられない。シミュレータを「そこに居る PLC」として
//! 外に立てれば、収集は**実際のワイヤ経路**（接続・読み取り・デコード）を
//! そのまま通るので一巡を確かめられる（製品の `simulation` は #413 で設定
//! できるようにしたが、値が記録されない約束は変えていないので、一巡の確認は
//! 引き続きこの別プロセスで行う）。
//!
