<script lang="ts">
	/**
	 * 設定画面のシェル（#359 chronogazer 分）。banto-hub（#371）の
	 * settings/+layout.svelte にならい、カテゴリナビ + 各カテゴリのルート
	 * （`{@render children()}`）をここに集約する。
	 *
	 * chronogazer には template の `PageHeader` コンポーネントが無く、
	 ...
	 * の doc comment参照）。banto-hub の `hubStatusStore` のような継続
	 * ポーリングは元々不要（元 `+page.svelte` も1回読むだけだった）なので、
	 * ここでは1回だけ `load()` する。失敗時は `authSettingsStore.error` に
	 * 記録する（元 `+page.svelte` の `authError` と同じ見た目を
