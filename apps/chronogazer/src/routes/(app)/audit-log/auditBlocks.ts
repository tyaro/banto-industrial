/**
 * 監査ログ一覧（`/audit-log`）の**ブロックキャッシュの判断**（#410）。
 *
 * `/events`（#409）と同じ欠陥を持っていた（修正前の `AuditLogWindow` を
 * 逐語コピーして vitest で再現済み - PR #410 の本文）:
 *
 * 1. **総件数が未取得だと要求を 1 本も出さない** - 初回取得が失敗した後も、
 *    正常に 0 件（絞り込み）になった後に絞り込みを外しても、`BantoGrid` が
 *    通知する表示範囲が `{0, 0}` なので `ensureRange(0, 0)` が空振りし、
 *    **回復の手段が無かった**（「再読み込み」ボタン自体も無かった）、
 * 2. **ブロックの合間に行が増える・消えると、境界で重複・欠落する**
 *    （監査ログは操作のたびに増え、一覧を読むたびに保持期間の削除が走る）、
 * 3. **失敗を状態として持っていない** - 失敗はトースト（4 秒で消える）だけで、
 *    未取得・取得失敗・正常な 0 件の区別も再試行の手段も無かった
 *    （`/events` の「別ブロックの成功が失敗を消す」とは形が違う - 共通の
 *    `error` がそもそも無かった）。
 *
 ...
 *   削除が続く間（保持件数の上限に張り付いて操作が続いているとき）に
 *   **一覧が永久に読み終わらない** - `/events` で「件数が増えたら取り直す」を
 *   採らなかったのと同じ理由。
