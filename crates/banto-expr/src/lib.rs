...
//! 照会が要る）は T6-2 の責務。
//!
//! ## 式ソースの文字数上限・パーサの再帰深さ上限（DoS 対策、2026-08-08）
//!
//! 演算タグを登録できる認証済みクライアントが `((((...))))` や
//! `-----...-1` のような深いネスト・連鎖の式を送ると、[`parser`][] の
//! 再帰下降パーサはソースの構造そのままの深さまで自身の呼び出しスタックを
//! 積む。`compile` は banto-hub 側で tokio のワーカースレッド上から呼ばれる
//! ため、そのスレッドの既定スタック（2MiB）を使い切るとスタック
//! オーバーフロー＝プロセス全体の abort になりうる（Rust のスタック
//! オーバーフローは `Result` は当然として `std::panic::catch_unwind` でも
//! 捕まえられない致命的エラーであり、1クライアントの1回の登録操作で
//! banto-hub プロセス全体を落とせてしまう）。
//!
//! 対策として `compile` の入口で2つの上限を設ける:
//!
//! - [`MAX_SOURCE_CHARS`][]: 式ソースの最大文字数。字句解析より前に検査する
//!   （超過時 [`CompileError::SourceTooLong`]）。
//! - [`MAX_NESTING_DEPTH`][]: パーサの再帰下降の深さ上限（超過時
//!   [`CompileError::TooDeep`]）。括弧のネスト・単項演算子の連鎖・関数
//!   呼び出し引数の3経路すべてを1つの深さカウンタで塞ぐ - 詳細は
//!   [`parser`][] モジュール doc 参照。
//!
//! いずれも「登録時（= `compile` 呼び出し時）に拒否する」という本クレート
//! 全体の設計方針（型不一致と同じ扱い - 本 doc の「型システム」節参照）に
//! 素直に従う。実行時評価（[`eval`][]）や型検査の再帰深さは常にパース済み
//! AST の深さと一致するため、[`eval`][] 側に別のガードは要らない - AST の
//! 深さは**2つの上限が共同で**抑えている: [`descend`][crate::parser] を
//! 通る3経路のネストは [`MAX_NESTING_DEPTH`][] が直接抑え、`descend` を
//! 通らない同優先度の二項連鎖（`1+1+...+1` は while ループで反復パース
//! される）は左傾斜の AST を項数ぶんの深さで作るが、その項数は
//! [`MAX_SOURCE_CHARS`][] により高々500程度に抑えられる。どちらの経路も
//! 2MiB スタックに対して余裕が2桁以上ある。
//!
//! ## DAG 検証
//!
...
//! - [`ast`][]: 構文木。
//! - [`parser`][]: 再帰下降パーサ（優先順位表つき・[`MAX_NESTING_DEPTH`]
//!   による深さガードつき）。
...
/// 式ソースの最大文字数（[`str::chars`] でカウント - バイト数ではない。
/// `compile` の入口、字句解析より前に検査する。
///
/// §4.2 の実用的な演算式（`docs/tag-server-design.md`）はどれも数十文字
/// 程度で収まり、1024 はそれに対して十分に寛大な上限 - この上限自体が
/// 実用上の制約になることは想定していない。存在理由は表現力の制限では
/// なく、深いネスト・長大な式による DoS を防ぐ2つの入口ガードの片方
/// （もう片方は [`MAX_NESTING_DEPTH`]、詳細は本 doc の「DoS 対策」節
/// 参照）。超過時は [`CompileError::SourceTooLong`]。
pub const MAX_SOURCE_CHARS: usize = 1024;

/// パーサ（[`parser`][]）の再帰下降の深さ上限。認証済みクライアントが
/// 演算タグ登録時に `((((...))))` や `-----...-1` のような深いネスト・
/// 連鎖式を送ると、この再帰下降パーサの呼び出しスタックがソース構造
/// そのままの深さまで積み上がり、tokio ワーカースレッド（既定スタック
/// 2MiB）でスタックオーバーフロー＝プロセス全体の abort になりうる
/// （本 doc の「DoS 対策」節参照）。登録時（= `compile` 呼び出し時）に
/// [`CompileError::TooDeep`] として拒否することでこれを防ぐ - 実行時
/// 評価（[`eval`][]）の再帰深さはパース結果の AST 構造そのものなので、
/// この上限が（[`MAX_SOURCE_CHARS`] と共同で - 本 doc の「DoS 対策」節
/// 参照）評価側の深さも抑える（`eval` に別途ガードを足す必要はない）。
/// 塞いでいる経路（括弧・単項演算子の自己再帰・関数呼び出し引数）の
/// 詳細は [`parser`][] モジュール doc 参照。
...
/// 検証するのは構文・演算子/関数の型規則・`bit()` の特殊制約に加えて、
/// 式ソースの文字数・パーサの再帰深さが安全な範囲内であること（本 doc の
/// 「DoS 対策」節参照）。
