...
//! 追加できる)。
//!
//! ## 外部から書き込まれた番地はランプ対象から外れる(#363、2026-09-15)
//!
//! オーナー決定 2026-09-15(#363)により、シミュレーション接続のタグへの
//! 外部書き込み(REST/gRPC/MCP)は拒否されず、この in-process シミュレータへ
//! 実際に反映される。ランプ波タスクは 100ms 周期で先頭
//! [`RAMP_ADDRESS_COUNT`]個の番地を無条件に上書きするので、そのままでは
//! 書いた値が次のポーリングまでに消えてしまう - そこで
//! `banto_plc::{modbus,slmp}::simulator::Simulator`側が「ワイヤ経由で
//! 書かれた番地」を**held**として記録し、ここで呼んでいる`set_*`
//! (ランプ/トグルの書き込み口)を held な番地に対して no-op にする
//! (詳細は各シミュレータのモジュール doc comment「Write support and the
//! held set」節)。したがってこのモジュール自身のループは #363 の前後で
//! 一切変わらない - 「書かれた番地だけランプが止まり、隣の番地は動き
//! 続ける」という挙動はシミュレータ側の held 判定だけで成立する。held は
//! シミュレータインスタンスと寿命を共にするので、接続の simulation 切替や
//! 全シミュレーション運転の開始・停止でシミュレータが作り直されれば消える。
//!
