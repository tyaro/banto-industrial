...
/// Put a row at `ptime_ms` straight into `file`'s table for `group_key`,
/// bypassing `TsWriter::append` - i.e. a row stored before the #424
/// ptime/file-date contract existed (when `append` took any `ptime_ms`),
/// which the read side must still handle. Table/column names come from the
/// file's own metadata (via `TsReader`), not hard-coded.
async fn insert_row_bypassing_the_writer(
    file: &std::path::Path,
    group_key: &str,
    ptime_ms: i64,
    value: f64,
) {
    let reader = banto_tstore::TsReader::open(file).await.unwrap();
    let meta = reader.group(group_key).expect("group in file").clone();
    drop(reader);
    ...
    let pool = sqlx::sqlite::SqlitePoolOptions::new()
        ...
        .await
        .unwrap();
    // AssertSqlSafe: the names are this test's own file's metadata
    // (`samples_<n>` / `c<n>`, generated by banto-tstore), not outside input.
    sqlx::query(sqlx::AssertSqlSafe(format!(
        "INSERT INTO {} (ptime, {}) VALUES (?, ?)",
        meta.table_name, meta.columns[0].column_name
    )))
    .bind(ptime_ms)
    .bind(value)
    .execute(&pool)
...
/// #423 review, finding 2: the file's name comes from the clock, not the
/// sample - so an ordinary 2026 file can hold a row at `PAST_OLD_CLAMP_MS`.
/// Every query whose range includes that time and the file's date must
/// return it (the clamp dropped it without an error) - the read side must
/// not narrow the range on its own.
///
/// #424: `TsWriter::append` now rejects such a row (asserted below), so the
/// row is put into the file directly, as data written before that contract
/// existed. Opening the writer with a clock at `PAST_OLD_CLAMP_MS` instead is
/// not possible: that is a year of about 73 million, which does not fit the
/// 8-digit `YYYYMMDD` file name, so `list_data_files` would not see the file.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn stored_row_past_the_old_clamp_is_still_found() {
    ...
    let err = writer.append("g1", t, &[Some(42.0)]).await.unwrap_err();
    assert!(matches!(
        err,
        banto_tstore::TstoreError::PtimeOutsideFileDate { .. }
    ));
    writer.close().await.unwrap();
    let files = banto_tstore::list_data_files(dir.path()).unwrap();
    assert_eq!(files.len(), 1);
    insert_row_bypassing_the_writer(&files[0].path, "g1", t, 42.0).await;
