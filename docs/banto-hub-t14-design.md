...
作成日: 2026-08-09
状態: **設計確定。主要判断 P1〜P6 を 2026-08-09 オーナー承認。** 実装未着手（T14-1 から着手可）。
...
どおりオーナー承認を経てから実装へ反映する。**主要判断 P1〜P6 は 2026-08-09 にオーナー承認
済み（§1）。**

## 1. 主要判断（オーナー承認済み）

以下は代替案のある設計判断で、実装範囲・共有クレート表面積・外部契約に影響する。詳細と根拠は
各節に。上位モデルが推奨を提示し、**P1〜P6 の全推奨を 2026-08-09 にオーナー承認**した。以後
変更する場合は §15 の運用どおり既存記録を上書きせず、新しい日付と理由を追記する。
...
> **2026-08-11 追補（方針改定）**: 公開操作の要件を「運転中編集ロック」から
> 「運転中編集はキュー化し、反映は手動適用、適用前はキャンセル可」へ改定する。
> これにより、運転中の CRUD/一括/CSV は `409` で一律拒否せず、pending queue へ
> 保存できる。一方、実行構成へ即時反映する経路は引き続き保護し、反映操作の
> 実行タイミングを明示的に制御する。`409` は「即時反映を要求したが許可されない」
> 場合に限定して用いる。実装計画と受け入れ条件の最新版は
> [banto-hub-desktop-plan.md](banto-hub-desktop-plan.md) §9.1 / §9.3 TAG-P0-3 を正とする。
