...
| `set_collection`                                  | POST /api/collection/start, /stop        | （2026-09-09 撤回、#340）write_enabled は変えない                |
...
- 収集中の登録変更は REST と同じく **pending queue（202 相当）**になる。MCP 応答でも
  「pending に入った」ことを明示する。**2026-09-14（#341）**: これは**ロックダウン
  済みのときだけ**の挙動になった - 試運転中（未ロックダウン）は収集中でも queue を
  経由せずその場で commit され、収集を止めずに実行構成へ反映される（判定は REST と
  共有の `rest::registry_change_should_queue`）。
