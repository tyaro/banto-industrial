...
- プロトコル別の例と即時検証は補助であり、自由入力とサーバー共通 preflight を
  維持する。
- 運転中でも Drawer で編集は可能とし、保存先は pending queue とする。
  画面には `未適用の変更` バッジ、`適用`、`キャンセル` を表示する。
  実行構成へ即時反映する API は `409 Conflict`（または専用エラー）で拒否し、
  キュー保存 API と明示適用 API を分離する。
...
- 運転中の構成変更は pending queue へ保存され、適用実行までは実行構成を変えない。
- 明示 `適用` でのみ preflight + 反映を行い、`キャンセル` で pending queue を破棄できる。
