...
| `get_tag` / `create_tag` / `update_tag` / `delete_tag`                                                                             | タグ                | get_tag は全フィールド＋`revision`（update の RMW 用）。delete は confirm。`list_tags` は §4 の read ツール。`tagKind: "db"` は postgres 接続配下専用で `address` は SELECT の結果列名・読み取り専用（S2）                                                                                                                                                                                              |
