...
管理面を含む 31 ツール全数を実機で end-to-end 検証済み（結果は §9、`test_saved_connection` 追加前）。2026-09-06 に外部 DB 連携（[banto-hub-external-db-design.md](banto-hub-external-db-design.md)）で `test_saved_connection`（32 ツール目、ローカル PostgreSQL 15.8 で検証）・接続の `database`/`username`/`password`・グループの `querySql`・`db` tag_kind を同期。構成補助（管理面）ツールは §6。
...
**2026-09-06（T21 管理面を実機で総当たり）**: 当時の main の 31 ツール（`test_saved_connection` 追加前）を実機
