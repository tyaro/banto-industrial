...
作成日: 2026-08-10
状態: **設計草案。主要判断 P1〜P6 は提案であり、オーナー未承認。実装は
承認後に着手する。** T16-0（薄いシェル）・T16-1（トレイ状態表示）は
マージ済みで本書の前提だが、T16-2（サービス検出・native fallback）は
本書 §4 の引き渡し契約が固まるまで着手できない。
最終検証日(コード照合): 2026-08-10
基準コミット: `801e862`（main、T18-1 TAG-P0-2 preflight エラー表示マージ後）。
...
**以下 P1〜P6 はすべて提案である。実装着手には各行の「オーナー承認」が
必要で、本書の記載だけでは着手できない**（[banto-hub-t16-design.md](banto-hub-t16-design.md)
の P1〜P3 が 2026-08-09 承認済みなのとは状態が異なる）。
...
§16.2（mutex 命名決定）、§16.3（T16/T17 未決事項）、および本書 §2 の現行
コード棚卸し。
...
**T16-2 はこの契約が本書で確定してから着手する**（P5）。契約の型・
エラー種別はここで決めた形を変えない前提で T17-0 を実装し、実装中に
形が変わった場合は本書を更新してから T16-2 に反映する。
...
- [ ] P1: profile path のモード非依存絶対パス一本化
- [ ] P2: Desktop↔Service 排他の3層併用（SCM state / named mutex / file lock 診断）
- [ ] P3: `BantoHub Operators` はサービス SDDL 直接付与、それ以外は UAC helper
- [ ] P4: `install()` の既定起動種別を `Demand` に変更
- [ ] P5: T17-0 が提供する API を T16-2 がそのまま消費する境界（§4）
- [ ] P6: 構成パッケージの秘密除外リスト（MQTT password / password_hash / key_hash）

**上記いずれも未承認。承認前の実装着手は行わない。**
