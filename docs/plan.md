...
## 4c. タグサーバーアプリ banto-hub（T系）— T0〜T4/T6〜T12 実装済み・残 T5（配布・運用強化・実機検証）
...
までを1つの製品 UI で完結させる。**初版は運転中の構成編集をロック**し（§4.3 の
オンライン部分再構成基盤は内部維持）、I1 CRUD の rebuild 失敗握り潰しを全構成
preflight へ置き換える。下表の T13-1 の続きは同計画の T14〜T18 が引き継ぐ。
...
| T5    | 配布・運用強化（サービス化検討・ソーク・インストーラ）                                       | T0〜T4  | 実機検証（多重クライアント）含む。T5-1(Windows サービス化)/T5-2(NSIS インストーラ)/T5-3(運用ガイド)は実装済み、残るは T5-5(72h soak 実行 + 実機最終サインオフ)。docs/banto-hub-operations.md §10/§11/§12、docs/t5-handoff.md 参照 |
