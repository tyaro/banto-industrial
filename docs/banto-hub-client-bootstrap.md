...
| `apps/chronogazer/core`（hub モジュール 43 本 + REST 4 本） | `installation_id` の生成と再利用、設定 KV に平文が無いこと、空選択の保存、切断、壊れた記録の扱い、`plan_bindings` の 8 分岐（未解決・購読不可・重複・空）、fingerprint が stable ID の変化で変わること、資格情報変更／終端状態での強制張り直し、起こすと張り直すが食い違わないこと、落ち着いた停止では catalog を撃たないこと、見張りが 1 本だけ起動すること・操作と直列化されること・レコード判定の前に hydrate すること、keyring 不可でも 6 状態が壊れないこと、`HubView` の JSON 形、`/api/hub/*` の admin 限定と未設定応答。**モック Hub（catalog → WS → REST values）で購読を実際に成立させ、値が画面の形まで届くこと・未知のラベルを丸めないことを固める 1 本**を含む |
