...
最終更新: 2026-09-25（#446: close 1008 の理由の保持・見張りの再試行の頻度・画面の案内を §10.5 / §10.6 に追加）
...
出す（案内だけ出して入力欄が無い、という形にしない）。トリップで接続側が `forbidden` になるのは
`banto-hub-bootstrap` が REST の 403（`key_tripped`）を権限不足と区別しないため（既存の挙動、
この変更の範囲外）。
