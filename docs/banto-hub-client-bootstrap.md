...
最終更新: 2026-09-25（#446: close 1008 の理由の保持・見張りの再試行の頻度・画面の案内を §10.5 / §10.6 に追加。接続の状態に `keyTripped` を足した（§5、wire 追加））
...
- **chronogazer**: 理由を `Subscription::credential_rejection` に覚え、**世代を止めた後も
  ...
  止めるが、そこで理由を捨てると画面が出し分けられない）。忘れるのは「受信できた（`Live`）」
  「キーを入れ直して Hub に接続できた」「切断した」とき。受け付けられなかったキーの採用では忘れない
  （保存されたキーは変わっていない）。REST（`GET /api/hub/subscription` ほか）と Tauri
