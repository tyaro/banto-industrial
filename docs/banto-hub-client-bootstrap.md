...
**動く条件**（保存済みレコードがあることが前提）:

| 現世代                        | 動くか | 理由                                                                                                                                                               |
| ----------------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 無い                          | ○      | まだ／もう張れていない。再計画で直る可能性がある（Hub が後から起動した場合など）                                                                                   |
...
**終端した世代は同一性が一致していても張り直す**（`Unauthorized` と、`last_error` 付きの `Stopped`）。
そうしないと見張りが再試行しても §10.1 の同一性比較で no-op になり、止まった世代が居座り続ける。
