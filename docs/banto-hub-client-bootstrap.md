...
## 5. 6 状態（+ `keyTripped`、#446）

> **2026-09-25（#446）で 7 つ目の `keyTripped` を足した**（wire: `{"state":"keyTripped"}`）。
> トリップしたキーは REST で `403 {"error":"key_tripped"}`（#435）を返すが、それまでは他の 403 と
> 同じ `forbidden`（「読み取り権限のあるキーを発行し、採用してください」）になり、購読の案内
> 「管理者に解除を依頼、同じキーのまま再開」と**逆のことを同時に言っていた**。403 のときだけ
> 状態確認の要求の本文を読み、banto-hub の固定のエラーコード `key_tripped` と一致するかだけを見る
> （本文はログにも保存にも出さない）。`connect()` は `keyTripped` のキーを**失効も再発行もしない**
> （`unreachable` と同じく、そこで止まる）。失効・期限切れ・存在しないキーは REST ではどれも同じ
> 401 なので、`authFailed` 1 つのまま（理由はストリームの close 1008 からしか分からない、§10.6）。
...
| `forbidden`              | 認証は通るが read スコープが無い（HTTP 403、`key_tripped` 以外）               | 管理者発行のキーを手入力して採用                             |
| `keyTripped`             | 保存済みキーがトリップしている（HTTP 403 `{"error":"key_tripped"}`、#446）     | 管理者に解除を依頼。**キーは捨てない**（同じキーで戻る）     |
...
- **最終受信時刻は `banto-tagclient` から導かず、こちらで覚える**。`current()` は `Live` を離れると
  ...
  を知りたい場面で消えることになる。live なスナップショットを見たときに記録する。観測するのは画面の
  ポーリングと見張りの各周期なので、**更新の粒度は最大 30 秒**。世代ごとに `watch` を購読する専用
  タスクは立てない（タスクを増やさずこの精度で足りる）。
- **残すか捨てるかは「同じ購読についての事実であり続けるか」で決める**。一律で捨てないのは、
  **いつまでデータが来ていたかが記録計の診断に一番効く**から。一律で残さないのは、**別の Hub・
  別のタグ集合の時刻を出すと嘘になる**から。判定は（正規化した接続先, ソート済みの選択タグ）という
  粗い同一性で、[`Fingerprint`](§10.1) と違って stable ID を含まない — catalog を読めない場面
  （切断中・認証エラー中）こそ「同じ購読が止まっているだけか」を判断したいため。

  | 状況                                                         | 最終受信時刻                                           |
  | ------------------------------------------------------------ | ------------------------------------------------------ |
  | 終端エラーで停止 / 再試行待ち / `Unauthorized` / `Rebinding` | **残す**（同じ接続先・同じ選択のまま止まっているだけ） |
  | 同じ接続先・同じ選択のまま張り直し                           | **残す**                                               |
  | 接続先が変わった / 選択タグが変わった                        | **捨てる**（別の購読になった）                         |
  | 切断（`disconnect`）/ レコードが無くなった                   | **捨てる**（その購読がもう無い）                       |

  画面は `live` でないときに「（購読は停止しています）」等を同じ行へ添える - 時刻だけを出すと、
  止まっているのに受信し続けているように読めてしまうため。
...
| `Unauthorized` + `last_error` が `Unauthorized`（401/403、理由無し）                                  | ○             | `banto-tagclient` にとって終端状態。放っておくと戻らないが、キーが差し替わっていれば直る                                                                                                                                                                                                                                |
| `Unauthorized` + `last_error` が `KeyTripped` / `CredentialRejected`（close 1008、#446）              | ゆっくり      | **2 分に 1 回**（4 周期に 1 回）。トリップは管理者が解除すれば**同じキーで戻る**ので、解除に気づけるよう続ける。解除までは同じ拒否が返るだけなので毎周期は撃たない。理由を判別できない 1008 も、戻るか分からないので止め切らない。§10.6                                                                                 |
| `Unauthorized` + `last_error` が `KeyRevoked` / `KeyExpired` / `KeyNotFound`（close 1008、#446）      | ×             | **同じキーでは二度と戻らない**。キーの入れ直し（`connect` / `adopt_manual_key`）が資格情報の変更として張り直す。**`Unauthorized` を状態名だけで「○」と決めない**（checklist §5）。§10.6                                                                                                                                 |
| 無い + close 1008 の理由を覚えている（#446）                                                          | 上 2 行と同じ | 世代を止めた後（設定画面の `status()` が catalog の 401/403 で止めた場合など）も、覚えている理由で同じ頻度にする。理由を忘れると 30 秒ごとに撃ち直す形に戻る。ただし「タグ未選択」「全部購読不可」の停止は理由に関係なく撃たない（張る要求が無い）                                                                      |
...
| `Reconnecting` + `last_error` が `BindingUnresolved` / `RevisionMismatch` / `RuntimeMetadataMismatch` | ○             | **`Reconnecting` は一律放置ではない**。`banto-tagclient` は再バインドの回数（3 回）を使い切ると `FailurePlan::Backoff` へ落ち、`Backoff` は `Reconnecting` + `last_error` を publish するだけなので、以後は**古い（存在しないタグを含む）要求セットのまま**永久に再試行する。catalog を読み直して要求を作り直すしかない |
...
| `Reconnecting` + `last_error` が通信系（`Transport` / `ProtocolError` / `CatalogUnavailable`）        | ×             | **`banto-tagclient` 側の backoff の仕事**。ここで stop → start すると backoff と喧嘩し、再接続を遅らせるか Hub を叩く回数を増やすだけ                                                                                                                                                                                   |

> **2026-09-17 実機で見つけた事故**: 選択中のタグを 1 つ Hub から削除したら、購読が
> `reconnecting` のまま `subscribedCount=3` / `unresolved=[]` で固まり、Hub を停止→再開しても
> `live` に戻らなくなった（残りのタグの値も流れない）。原因は上表の `Reconnecting` を**状態名だけで
> 一律「backoff の担当」として除外**していたこと。**下位ライブラリは同じ状態名に別々の意味を載せる**
> ので、分類（`last_error`）まで見る。

**起こすなら実際に張り直す**（世代があるかぎり例外なし）: 「見張りが起こすか」と「同一性が一致して
いても張り直すか」は同じ述語を共有する。片方だけ真だと、catalog を取り直しても §10.1 の同一性比較で
no-op に落ち、壊れたワーカーが残る。**この形のバグは 3 回出た** — `Rebinding`（#385）、
`Reconnecting` + binding 系（#383 実機）、`Reconnecting` + メタデータ不一致（#388）。1 つの述語を
両方から使い、全状態 × 分類を表でテストすることで構造的に入らないようにしてある。世代が無いときだけは
張り直しの話にならない（張るものが無く、突き合わせが新しく張る）ので対象外。

**「古い要求セットのまま `Reconnecting`」も張り直す**。当初は「catalog が変われば同一性も変わるので
自然に張り直る」と考えて張り直しの判断から外していたが、**それが成り立つのは `BindingUnresolved`
だけ**だった:

- `BindingUnresolved`（タグが消えた）… 再計画すると要求セットが変わる → 同一性が変わって張り直る
  （実機で確認済み: 削除から約 15 秒で `live` に復帰し、残りのタグは流れ続けた）。
- `RevisionMismatch` / `RuntimeMetadataMismatch` … **タグ集合は同じまま**なので同一性が変わらず、
  早期 return に落ちて**見張りが 30 秒ごとに catalog を読むだけ**になる。ワーカーは壊れたループのまま。

ワーカー自身が「要求セットが catalog と食い違っている」と言っている以上、同じ要求のまま待たせても
直らない。張り直せば catalog を読み直して要求を作り直すので、メタデータ不一致の 2 つも回復できる。
条件が続く間は 30 秒ごとに張り直すことになるが、**永久に止まっているよりはよい**（周期で上限が
付いている）。

これは §10.1 の [`Fingerprint`] に catalog の `revision` / `run_id` / `collection_mode` を
**含めない**という選択と対になっている: 含めると Hub 側の無関係な構成変更のたびに購読を切って
しまうので含めず、そのぶん「メタデータ不一致で詰まった世代」はこちらで拾って張り直す。

### 10.6 Hub がキーを使えないと言って購読を打ち切ったとき（#446）

banto-hub は開いているストリームのキーを 15 秒ごとに照合し直し（#430）、使えないと**確認できたら**
close コード `1008` と理由文（`api_key_revoked` / `api_key_expired` / `api_key_tripped` /
`api_key_not_found`、#439）で閉じる。#446 までは `banto-tagclient` がこれを通常の切断として扱い、
1 回余計に再接続して catalog の 401/403 で `Unauthorized` として止まっていた（理由が消える）。

- **`banto-tagclient`**: close コードと理由文を分類し（`close::classify_close`、純関数・表でテスト）、
  `1008` なら `Unauthorized` + 理由ごとの `last_error`（`key_revoked` / `key_expired` /
  `key_tripped` / `key_not_found`、未知の理由は `credential_rejected`）で止まり、**再接続しない**。
  `1008` 以外（`1000` / banto-hub の backpressure の `1013` など）は従来どおり再接続する。理由文は
  banto-hub の `stream.rs` と二重に書いているので、`banto-tagclient` のテストが Hub のソースを読んで
  綴りと close コードの一致を固定する。
- **chronogazer**: 理由を `Subscription::credential_rejection` に覚え、**世代を止めた後も
  `subscription.lastError` に出し続ける**（設定画面を開くと `status()` が catalog の 401/403 で世代を
  止めるが、そこで理由を捨てると画面が出し分けられない）。理由には**どのキーの話か**を添え、
  今保存しているキーの話のときだけ使う（下の「別のキー・別の時点の判定を混ぜない」）。REST（`GET /api/hub/subscription` ほか）と Tauri
  （`hub_subscription` ほか）は同じ `HubSubscriptionView` を返すので、どちらの経路でも同じに見える。
- **見張り**: §10.5 の表の #446 の行。

| `lastError`           | 画面の案内（`hubCredentialGuidance`）                                                                                    | 戻り方                                                                                     |
| --------------------- | ------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------ |
| `key_tripped`         | Hub の管理者に解除を依頼してください（同じキーのまま、数分以内に自動で再開）                                             | 管理者が解除 → 見張りが 2 分以内に張り直す。設定画面を開けば `status()` がその場で張り直す |
| `key_revoked`         | 新しい API キーを設定してください（「接続」で再発行 / Hub の管理画面で発行したキーを採用）。同じキーでは自動で再開しない | キーの入れ直し（`connect` / `adopt_manual_key`）で張り直す                                 |
| `key_expired`         | 同上（有効期限切れ）                                                                                                     | 同上                                                                                       |
| `key_not_found`       | 同上（Hub にキーが見つからない）                                                                                         | 同上                                                                                       |
| `credential_rejected` | Hub の管理者に確認してください（理由を判別できない）                                                                     | 見張りが 2 分に 1 回確かめる                                                               |

`unauthorized` のときは説明文そのものが案内になり、世代を止めた後（`stopped` + 停止の理由）は
停止の理由の下に案内を別の行で出す。失効・期限切れ・存在しないは、止めた後も手動キーの入力欄を
出す（案内だけ出して入力欄が無い、という形にしない）。

**接続の状態と購読の案内を食い違わせない**（#446 の監査で追加）。接続の状態（REST の答え）は
今のキーについての最新の判定なので、食い違うときはそちらに合わせる。バックエンド
（`credential_rejection_after_status`、覚えている理由を直す）と画面（`effectiveCredentialGuidance`）の
両方で同じ向きにそろえ、vitest が「接続の状態 × 購読の状態 × `lastError`」の全組み合わせで
「解除を依頼（キーを捨てるな）」と「キーの採用・再発行・入力欄（キーを替えよ）」が同時に出ない
ことを固定する。

| 接続の状態                                    | 覚えている理由・画面の案内                                                                                      | 入力欄                     |
| --------------------------------------------- | --------------------------------------------------------------------------------------------------------------- | -------------------------- |
| `keyTripped`                                  | 理由を `key_tripped` にする（1008 を受けていなくても）。案内は接続の状態の説明が兼ねる。見張りはゆっくり        | 出さない                   |
| `authFailed`（401）                           | `key_tripped` の記憶は捨てる（もうトリップではない）。失効・期限切れ・存在しないはそのまま（1008 の方が詳しい） | 失効などなら出す           |
| `forbidden`（`key_tripped` 以外の 403）       | 失効・期限切れ・存在しない・トリップの記憶は捨てる（キーは認証できている）                                      | 出す（従来どおり）         |
| `needsPairing`                                | トリップの案内は出さない                                                                                        | 出す（従来どおり）         |
| `connected` / `unreachable` / `notConfigured` | 購読の理由の案内のまま（トリップの購読だけが止まっている場合は入力欄を出さない）                                | トリップ以外の拒否なら出す |

**別のキー・別の時点の判定を混ぜない**（2026-09-26、#449 のオーナーレビュー P2 3 件）。3 件とも
「別のキー・別の時点についての判定を、今保存しているキーの記憶に混ぜる」という同じ型だった:
新しいキーを保存したのに最後の検証だけが失敗すると古いキーの失効の理由で再試行を止め続ける
（P2-1）、採用しなかった候補キーの判定で保存中のキーの理由を上書きする（P2-3）、画面が持つ古い
接続の状態で新しい失効の理由を抑える（P2-2）。

- **判定の対象を名指しする**: `banto-hub-bootstrap` の `HubConnection::judged` は、その往復で実際に
  catalog に当てたキー（接続先 + キー）のプロセス内の識別子 `CredentialIdentity`（プロセスごとに
  乱数の種を持つ鍵付きハッシュ。保存しない・ログに出さない・キーには戻せない）。
  `Bootstrapper::stored_credential()` は**今保存している**キーの識別子（設定と keyring を読むだけ）。
- **覚えている理由にも対象のキーを添える**（`RememberedRejection`）。close 1008 の理由は、その世代を
  張ったキーの話として覚える。
- **記憶を書き換える経路**（chronogazer の `hub.rs`）:

  | 経路                                                                                                            | 書き換え                                                                                                                                                              |
  | --------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
  | 世代の観測（画面のポーリング・見張りの各周期・世代の停止）                                                      | 世代の close 1008 の理由を、**その世代のキー**の話として覚える。`Live` なら忘れる                                                                                     |
  | 突き合わせの最後（`status` / `refresh_catalog` / `connect` / `adopt_manual_key` / 選択の保存 / 見張りの再試行） | 覚えている理由の対象が今保存しているキーでなければ捨てる（入れ替わった・切断した）。REST の判定は、**判定の対象が今保存しているキーのときだけ**表（上）どおりに当てる |
  | 採用しなかった候補キーの `adopt_manual_key`（判定の対象 ≠ 保存中のキー）                                        | **何も触らない**（世代・停止理由・覚えている理由のどれも）。以前は候補の判定で**受信中の購読まで止めていた**                                                          |
  | 見張りの各周期（突き合わせの前）                                                                                | 覚えている理由の対象が今保存しているキーでなければ捨てる。`connect` が新しいキーを保存した後で打ち切られた・失敗した場合もここで拾う                                  |
  | 打ち切った往復（`HubConnection::failed`、`judged: None`）                                                       | REST の判定としては使わない（キーが入れ替わっていれば、上の 2 つが捨てる）                                                                                            |

- **画面**: 接続の状態は画面を開いたとき・明示操作のときにしか取り直さない（`status()` は Hub へ往復
  するのでポーリングしない）。そこで接続の状態を取ったときに一緒に届いた購読の状態を覚えておき、
  購読のキーに関わる部分（close 1008 の理由・理由の無い拒否・受信中）が変わったら、キーについて
  言っている接続の状態（`keyTripped` / `authFailed` / `forbidden` / `needsPairing`）は**古い**として
  「確認し直しています」を出し、案内と入力欄の判断に使わず、1 回だけ取り直す（`hubStatusDisplay`）。
  採用しなかった候補キーの応答は、接続の状態を前のまま残し、採用できなかった理由として別に出す
  （`adoptionResult`）。
- **画面の状態を書き換える経路**（2026-09-26、#449 の再レビュー P2 ×2）。接続の状態・その観測時点・
  確認し直しの段階は 1 つの組（`HubStatusSnapshot`）で、必ず一緒に書き換える。片方だけ新しくすると、
  古い接続の状態を新しい購読と一緒に観測したことになり、古さを見落とす。

  | 経路                                                                               | 観測するもの                                   | 更新するもの                                                                                                   |
  | ---------------------------------------------------------------------------------- | ---------------------------------------------- | -------------------------------------------------------------------------------------------------------------- |
  | 画面を開いたとき・接続・一覧の更新・切断・確認し直し・打ち切り後の読み直し（成功） | 保存しているキーについての `status()` 系の応答 | 組ごと（`snapshotFromStoredView`）と購読                                                                       |
  | 手動キーの採用（採用できた）                                                       | 同上（保存したキー）                           | 組ごとと購読                                                                                                   |
  | 手動キーの採用（採用できなかった）                                                 | 候補キーの判定（保存中のキーの話ではない）     | **組は変えない**（観測時点も、確認し直しの失敗も残す）。購読と「採用できませんでした」だけ                     |
  | 購読のポーリング・選択の保存後の読み直し                                           | 購読だけ                                       | 購読だけ（組は変えない。古くなったかは組の観測時点と比べて決める）                                             |
  | 確認し直し（`runStaleRecheck`）の開始                                              | 古くなった（購読のキーに関わる部分が変わった） | 段階を `pending`。`busy`                                                                                       |
  | 確認し直しの失敗・打ち切り・reject                                                 | 取り直せなかった                               | 段階を `failed`（「接続の状態を確認できませんでした」と読み取り専用の「状態を再取得」を出す）。`busy` を降ろす |
  | 確認し直しの自動の再試行                                                           | 購読が届くたびに評価                           | 失敗から 30 秒経つか、購読のキーに関わる部分が変わったときだけ `pending`（連続では撃たない）                   |
  | 明示操作・読み直しの失敗                                                           | -                                              | 組は変えない（従来どおりエラーの表示・`statusUnconfirmed`）                                                    |

**往復は成功したが中身は失敗（2026-09-26、#449 の 3 回目のレビュー）**。`Bootstrapper::verify` は catalog の
503 や通信障害を `Ok(HubConnection{ status: Unreachable })` で返すので、画面→アプリの往復が成功しても、
Hub に届いたとは限らない。確認し直しがこれを「済んだ」と扱うと、到達不能のまま再試行も「状態を再取得」も
消えた。

- 確認し直しの答えが「接続済み」でなければ、答えは接続の状態として出す（組ごと新しくする）が、段階は
  `failed`（`answered: true`）にして、間隔（30 秒）を守った再試行と「状態を再取得」を残す。
- **購読が受信しているのに接続の状態が「接続済み」でない**のは、どの状態でも古い（`isHubStatusStale`）。
  購読の受信が、Hub が戻ったことを画面が知る唯一のきっかけ（`status()` はポーリングしない）。
- 「状態を再取得」は、確認し直しに失敗しているとき、または接続の状態が「Hub に到達できない」で保存済みの
  接続先があるときに出す（`showStatusRecheckButton`）。古くなくても取り直せる。

| 往復は成功した答え（`connected` 以外） | 画面を開いた・明示操作・読み直し                                                     | 確認し直し                                                                     | 手動キーの採用（候補）                   |
| -------------------------------------- | ------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------ | ---------------------------------------- |
| `unreachable`                          | 表示する。「状態を再取得」（保存済みの接続先があれば）。購読が受信したら取り直す     | 表示し `failed`（答えあり）。30 秒ごと・購読の変化・「状態を再取得」で取り直す | 組は変えない（「採用できませんでした」） |
| `authFailed`                           | 表示する。「接続」で再発行・キーの入力欄（失効などの案内）。購読が受信したら取り直す | 同上                                                                           | 同上                                     |
| `forbidden` / `needsPairing`           | 表示する。キーの入力欄。購読が受信したら取り直す                                     | 同上                                                                           | 同上                                     |
| `keyTripped`                           | 表示する（管理者に解除を依頼）。見張りがゆっくり張り直し、購読が受信したら取り直す   | 同上                                                                           | 同上                                     |
| `notConfigured`                        | 表示する（接続先を入れて「接続」）                                                   | 同上                                                                           | -                                        |

## 11. 実機確認（2026-09-17）

**構成**: 使い捨ての banto-hub（`127.0.0.1:8722`、試運転モード = 未ロックダウン、loopback バインド）＋
実機 PLC **R08ENCPU**（`192.168.11.200:5200`、SLMP、安全番地 D3000-4999 / M1000-2000）。
chronogazer 側は **Tauri のウィンドウを自動操作できない**ため、同じサービス層
（[`HubService`](../apps/chronogazer/core/src/hub.rs)）を直接叩く
`apps/chronogazer/core/examples/hub_real_smoke.rs` で確認した（GUI そのものは未確認）。

| 確認したこと                                                  | 結果                                                                                             |
| ------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| 試運転中の Hub へ `read` キーを自己発行（手入力なし）         | OK                                                                                               |
| **タグ 0 件の Hub でも接続設定を完了できる**（#332 受入条件） | OK（`connected` / `tag_count: 0`）                                                               |
| 実 PLC 値の購読                                               | OK（`q: good` / `value_source: real`、live 到達 1.0 秒）                                         |
| 値の変化への追従                                              | OK（**変化したタグの `t` だけ進む** = 変化駆動の配信）                                           |
| アプリ再起動でキーを再発行しない                              | OK（key_name・keyring account・平文キーとも同一、1.0 秒で復帰）                                  |
| 選択タグの 1 つが Hub から消えても残りが流れる                | OK（**§10.5 の修正後**。15〜22 秒で再計画し `unresolved` に出す）                                |
| Hub 停止 → 再開で自力復帰                                     | OK（**§10.5 の修正後**。`lastError=transport` のまま backoff に委ね、再開の約 12 秒後に `live`） |
| Hub の書き込みレート制限ブレーカ                              | 設計どおり作動（連続書き込みで `key_tripped`）                                                   |
| プロファイル排他ロック（二重起動防止、T17-1）                 | 設計どおり作動                                                                                   |

### 実機でしか出なかった欠陥（#388）

**モック相手のテストは全て緑で、Copilot レビュー 11 往復も通過していた**が、実機で 2 件出た。

1. **タグが 1 つ消えると購読が二度と戻らない**。`banto-tagclient` は再バインドの試行回数を使い切ると
   `Reconnecting` + `last_error = BindingUnresolved` に落ち、**古い要求セットのまま**再試行し続ける
   （`worker.rs` の `plan_failure` → `FailurePlan::Backoff`）。見張りは `Reconnecting` を状態名だけで
   「transport の backoff の担当」として一律に除外していたため、再計画が永久に起きなかった。
   実機では削除後 60 秒以上復帰せず、Hub を再起動しても戻らなかった。
2. **メタデータ不一致では張り直せない**。`RevisionMismatch` / `RuntimeMetadataMismatch` はタグ集合が
   変わらないので同一性も変わらず、見張りが起きても早期 return で何もしなかった（1 の修正直後に
   レビューで発見）。

いずれも「**起こす条件**と**実際にやる条件**が別々に列挙されていた」ことが原因で、§10.5 で 1 つの
述語に統合し、「起こすなら必ず張り直す」を全状態の表でテスト固定した。

### 未確認

- **設定画面（Tauri の GUI）そのもの**。上記はサービス層までの確認で、画面の操作性・表示は含まない。
- **`NeedsPairing`**（ロックダウン済み Hub での手動キー連携）。ロックダウンには管理者アカウントの
  作成が必要で、アカウント作成とパスワード入力は Claude が行わない運用のため。状態自体は
  `banto-hub-bootstrap` のモック付きテストで固定済み。
