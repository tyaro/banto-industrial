...
C-1〜C-4 に分割して着手中で、C-1・C-2・C-3a が入った**（2026-09-20。内訳は
...
**この完了条件を満たすのは C-4 の一巡が通ったとき**で、C-1/C-2/C-3a が入った
だけでは「動く」とは言えない。
...
| C-3b | 画面（収集の状態表示と操作、現在値表示、`collect_events` のイベント一覧ページ）                                                                                                                                                             | **未着手**           |
| C-4  | シミュレータ・ハーネスと E2E（設定 → 収集開始 → データファイル生成 → イベント記録の**一巡**）                                                                                                                                               | **未着手**           |

C-3a 時点の既知の制約（いずれも設計判断として記録済み。詳細は
