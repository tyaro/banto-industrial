...
C-1〜C-4 に分割して着手中で、C-1・C-2 が入った**（2026-09-20。内訳は
[R1-C の節](#r1-c-収集ランタイム統合)）。**まだ一巡は通っていない** -
...
#### 実施の分割（#383 段階2b、2026-09-20 時点）

**この完了条件を満たすのは C-4 の一巡が通ったとき**で、C-1/C-2 が入った
だけでは「動く」とは言えない。
...
| C-3 | 画面（収集の状態表示と操作）、現在値 API（`CurrentValuesHandle` → REST ポーリング + Tauri）、`collect_events` のイベント一覧ページ                                             | **未着手**           |
| C-4 | シミュレータ・ハーネスと E2E（設定 → 収集開始 → データファイル生成 → イベント記録の**一巡**）                                                                                  | **未着手**           |

C-2 時点の既知の制約（いずれも設計判断として記録済み。詳細は
`apps/chronogazer/core/src/collect.rs` のモジュール doc）:
