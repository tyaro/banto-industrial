...
コード署名／WebView2 Fixed Version 同梱、NSIS「インストール後に実行」チェックボックス、
UAC split-token 判定（見送り済み）、OPC UA、MQTT 組み込みブローカー、タグブリッジ・スクリプト、
ux-plan §5 バックログ 4 件（スパークライン／イベント一覧／手動書き込み／Swagger UI 同梱）、
API キースコープ配列上限、一回限り ticket プロトコル、仮想サービスアカウント採否。
