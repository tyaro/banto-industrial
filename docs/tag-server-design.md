...
作成日: 2026-08-04
状態: **実装追従中（2026-09-24 更新: §6 の 6 項に、書き込み受付の停止を DB と状態ファイルの
...
以下は経緯。2026-09-17 更新: §7 に ChronoGazer 3ドライバ構成のオーナー決定（#383）を追記し、
§5.6 の #332 の記述から relay-wright（凍結）を外した。同日追補: #383 段階1（ChronoGazer が選んだ
Hub タグを購読して値を受ける）の完了を §7 の移行表に反映。2026-09-16 更新: #342 段階B のセグメント補完と
`GET /api/tags/expression/functions` を §4.2 に追加＝#342 は A/B/C 完了。
以下は経緯。2026-09-15 更新: #335 - computed タグ・接続単位シミュレー
...
セグメント補完（トリガー・確定キー・候補の出所と除外・`Esc` の優先順位）と、
組み込み関数表を `banto-expr` 単一ソースにして配る
`GET /api/tags/expression/functions` を §4.2 に追加 - これで #342 の
A/B/C が揃った**。
起案時は設計先行だったが、
...
- **条件付き・自動書き込み**（ルールエンジン）: relay-wright が構想していた領域（同アプリは
  凍結後、2026-09-24 に main から退避済み）。タグサーバーの
...
**セグメント補完（2026-09-16、#342 段階B、v0.2.0-alpha.19）**: 完全名を
手で打つ手間そのものを減らすため、式欄でキャレット直前の字面を見て
**接続名 → 収集グループ名 → タグ名**の候補をポップアップで出す。これで
#342 の3段階（A: 式チェック API / B: セグメント補完 / C: 一覧から挿入）が
揃った。

- **開くきっかけ**: ドットの入力 / 2 文字以上の入力 / **`Ctrl+Space`** /
  **`Ctrl+.`**。`Ctrl+Space` は Linux の ibus や Windows の IME が入力ソース
  切り替えに取ることがあってアプリまで届かない環境があるため、`Ctrl+.` を
  併用できるようにしてある。**IME 変換中は開かない**（段階Aのチェックと
  同じ `compositionstart`/`compositionend` の抑止を共有する）。**閉じる**のは
  `Esc` / キャレット移動 / 外側クリック・スクロール・リサイズ / フォーカス喪失
  に加えて、**いま打っているトークンを終わらせる文字**（空白・演算子・括弧など）
  を入れたとき。いずれの場合も、次に打鍵するまで自動では開き直さない
  （`Ctrl+Space` / `Ctrl+.` はいつでも開ける）。
- **確定**: `Enter` / `Tab` / クリック。移動は `↑` `↓`。接続・グループを
  確定すると**名前 + `.`** を入れて**そのまま次の階層を開く**、タグを確定
  すると名前だけ入れて閉じる、関数は `name(` を入れて閉じる。挿入は「一覧
  から挿入」と同じ作法（`setRangeText` + `input` イベント再送）なので、
  確定のたびに上の式チェックがそのまま走る。
- **`Esc` の優先順位**: ポップアップが開いている間の `Esc` は**ポップアップ
  だけ**を閉じ、「一覧から挿入」トグルは ON のまま残す。閉じているときの
  `Esc` は従来どおりトグルを OFF にする（ペインは閉じない）。
- **候補の出所**: 接続・グループ・タグはタグ画面が持つフラット3配列から
  組んだ索引（接続名 → グループ名 → タグ。一覧が変わったときだけ組み直す）。
  `calc` / `mem` は**特別扱いしない** - 予約された仮想接続名ではあるが、
  式から見ればどれも「第1セグメント＝接続名」でしかない。
- **候補の除外**: 式で表せない名前（上の「一覧から挿入」と同じ識別子規則）に
  加え、第3セグメントでは段階Cの除外4種（自タグ・式で表せない名前・文字列型・
  循環）を**そのまま**使う（判定は `expressionInsert.ts` の同じ純関数）。
  ただし段階Cの行クリックが「淡色＋クリック時にトースト」なのに対し、
  **補完では最初から候補に出さない** - 補完は打鍵の続きに出るものなので、
  選べない候補は矢印キーで踏むぶんだけ邪魔になる（理由を知りたい人には
  一覧側の淡色表示が残っている）。
- **判定は式全体をパースしない**。キャレット直前を後ろ向きに1回走査するだけで、
  **空白を跨がない**（`conn . group . tag` は lexer 上は有効な参照だが、補完は
  打鍵中の連続した字面だけを見る）。数値の途中・4セグメント目では開かない。
- **組み込み関数の表は `banto-expr` を単一ソースにして API で配る**:
  `banto_expr::BUILTIN_FUNCTIONS`（名前・引数個数・呼び出し形・1行説明。
  型検査の `check_call` が名前と引数個数を引くのと**同じ表**）を公開し、
  **`GET /api/tags/expression/functions`**（`POST /api/tags/expression/check`
  と同じ `require_editor`、応答は
  `{ "functions": [{ "name", "arity", "signature", "description" }] }`）が
  それを配る。TS から Rust の const は import できないので、フロントに関数表を
  書き写す（＝2つ目の表を作ってドリフトさせる）代わりにサーバーから配る形に
  した。内容は静的なのでクライアントは1回取得してキャッシュし、**取得に失敗
  してもタグ候補だけで補完は動く**。関数候補は第1セグメントのときだけ接続名の
  後ろに混ぜる（式の項の先頭は接続名も関数名も同じ識別子のため）。**MCP には
  追加しない**（補完は UI 専用）。
- **補完は狭幅の中央モーダル・Drawer でも効く**（キーボードだけで完結し、
  グリッドを使わないため）。広幅の右ペイン限定である「一覧から挿入」トグルとは
  この点が違う。
- **contenteditable 化は採らない**（issue #342 の「見送り」のとおり。
  `textarea` + ポップアップのまま）。キャレット座標は段階Aのエラー位置下線で
  使っている `.expr-mirror`（textarea と box model が完全一致するミラー）を
  そのまま流用して測る。
...
- **Banto アプリ側の自動接続（#332）**: chronogazer は試運転中に
...
relay-wright が構想していた領域で、同アプリは 2026-09-24 に main から退避済み）。
...
6. **書き込み受付の既定と永続（2026-09-09 オーナー決定で撤回・変更、#340）**:
   ...
   **実機確認（2026-09-14、R08ENCPU、alpha.9）**: 起動直後 `writeEnabled: true`、収集開始後も
   ...
   有効復元。
   **永続先を 2 か所に（2026-09-24、#433）**: #340 の時点では永続値は DB（`write_control_state`）
   1 か所だけで、停止を DB に保存できないと再起動で有効に戻った。#433 で DB とデータディレクトリの
   状態ファイル `write-control-state.json` の両方に記録し、起動時は「どちらか一方でも停止（または
   読めない）なら停止」とした。停止はどちらか一方に保存できれば成功（DB 保存は 5 秒で打ち切る）、
   再開は両方に保存できたときだけ。運用上の扱いは `docs/banto-hub-operations.md` §4、設計の詳細は
   `apps/banto-hub/core/src/write_control.rs` のモジュール doc。
...
**2026-09-17 追記（オーナー決定）**: ChronoGazer は単体で動く記録計として **SLMP / Modbus TCP /
banto-hub 経由**の3ドライバを持つ（plan.md §4、#383）。(A) 型は「Hub があるときの形」であって
必須ではない。下表の「ChronoGazer リモート収集モード」は v2 候補ではなく**段階的に着手する**
（Hub 経由ドライバ = #383 段階1、購読まで実装済み）。「relay-wright の読み取り委譲」は relay-wright 自体の凍結に
伴い保留。
...
| 段階    | 構成                                                | 説明                                                                                                                                                                                                                                                                                                                                                                                 |
| ------- | --------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 現状    | PLC へ直接つなぐのは banto-hub と relay-wright だけ | ChronoGazer は **banto-hub 経由の購読（#383 段階1）で値を受けるところまで実装済み**で、PLC への直結（SLMP / Modbus TCP = 段階2）は未着手。したがって PLC のセッションを消費するのはこの2つのまま。単独稼働なら問題なし。同居時はセッション数に注意（W5 実機検証は relay-wright の凍結で保留）                                                                                        |
...
| 着手    | ChronoGazer の banto-hub 経由ドライバ               | 2026-09-17 決定で v2 候補から格上げ。**段階1（購読して値を受ける）は実装済み** — `TagClientHandle` の世代を ChronoGazer が所有し、同一性は「接続先 + タグ集合」。未解決タグは残りだけで購読して一覧に出す。SLMP / Modbus TCP の直結（段階2）と現在値ビューへの合流（段階3）は未着手。「収集をタグサーバーへ全面委譲し UI と収集の寿命を分離する」完全な (A) 型モードは引き続き先の話 |
| 保留    | relay-wright の読み取り委譲                         | 条件評価の入力読み取りをタグサーバー購読 (A) へ。**書き込みセッションは relay-wright 専有を維持**（安全機構と一体のため、(B) の恒久例外）。**relay-wright 自体が 2026-09-17 に凍結**（plan.md §4b）                                                                                                                                                                                  |
