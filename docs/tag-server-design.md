...
**2026-09-08 更新（issue #337）**: §6 項目5 の「Modbus は収集読み取りを直接クライアントに
残し2ソケットになるトレードオフを許容する」という #131 当時のオーナー決定を**撤回**し、
Modbus TCP の収集読み取りも broker セッションへ相乗りさせた（**1接続 = 1ソケット**）。
1対1接続しか受け付けない実機（オムロン KM-D1-ETN）で収集が一切成立しなかったため。
...
（FC5/6/15/16）のエンコード・実行を追加、`crates/banto-plc` の Modbus
フレームヘルパーを必要最小限だけ公開。broker 配線（#130 完了後の次スライス）
まで、Modbus タグへの書き込みは引き続きアプリ層で拒否される。
**同日追記（#131 後半スライス）**: broker 配線を実装し、Modbus TCP 接続
配下のタグの Numeric（数値/ビット、FC5/6/15/16）書き込みが実際に使える
ようになった（詳細は §6.2 末尾の追記）。String / BitInWord は引き続き
スコープ外（個別リクエストが `Bad` になる）。Modbus の**収集読み取り**は
このスライスでは変えていない — 引き続き `banto-collect` 自身の直接
`ModbusTcpClient` を使う（書き込み・ステータス表示だけが broker 経由）。
...
- **ロガー・日報・帳票機能**（FA-Server の FA-Logger / FA-Report 相当）:
  ...
  あって製品機能としてのロガーではない
  **2026-09-06 追記（オーナー決定）**: 外部 RDB へタグ値を流す **DB Sink**
  （Issue #229）に限り、この決定を部分的に覆す。位置づけは「方針を持たない
  出口」（MQTT 発行と同列）で、Hub は履歴を持たず読み返さず、保持期間・
  間引き・wide 形式・帳票は永久に非対応。エンジンは別プロセスのサイドカー、
  設定・UI・監視は Hub が持つ。理由と詳細は
  [banto-hub-external-db-design.md](banto-hub-external-db-design.md) §2.1 / §5
...
**一覧から挿入（2026-09-15、#342 段階C、v0.2.0-alpha.18）**: 式欄の完全名
（`{接続}.{グループ}.{タグ}`）は手で打つには長く、打ち間違いはそのまま
`unknown_tag` になる。式欄の直下に「一覧から挿入」トグルを置き、**ON の間だけ**
タグ画面中央のグリッドの行クリックが「そのタグの完全名を式欄のキャレット位置へ
挿入」になる（編集対象は切り替わらない。挿入後もフォーカスは式欄に残り、上の
式チェックがそのまま走る）。OFF の間の行クリックは従来どおり（編集を開く／
選択モードなら選択のトグル）。

issue #342 C の原案は「`ConnectionTree` でタグをクリックして挿入」だったが、
**ツリーの3階層化は採らなかった**（2026-09-15 オーナー決定）。`ConnectionTree`
は「接続 → 収集グループ」の2階層でタグを出さない設計（T18-6c）で、そこへ
タグ階層を足すのは T18-6 の決定を覆す大きな変更になる。一方 #375
（v0.2.0-alpha.17）でタグの編集フォームが**非モーダル**の右ペインになり、
編集中も左ツリーと中央グリッドをそのまま操作できるようになったため、
「ツリーでグループを選ぶ → グリッドに出た行をクリックする」で原案と同じ操作が
成立する。段階Cではあわせて新規作成フォームも同じペインへ移した
（[banto-hub-desktop-plan.md](banto-hub-desktop-plan.md) §9.4 TAG-UX-C の
2026-09-15 追補その3）- トグルはフォームがペインに出ているときだけ表示し、
狭幅のモーダル・Drawer では出さない（オーバーレイの下のグリッドを触れないため）。

挿入できないタグ（クリックしても挿入せず、理由をトーストで出す）は
**自タグ・式で表せない名前のタグ・文字列型タグ・（推移的に）自タグを参照して
いる演算タグ（＝循環）**の4つ。2つ目はレジストリのタグ名・接続名・グループ名の
検証（空でない・最大長）が本文法の識別子より広いため - 式に書けるのは
**3セグメントそれぞれが「英字か `_` で始まり、ASCII 英数字・`_`・`-` が続く」**
名前だけで、しかも**ハイフンは後ろに識別子継続文字が続くときだけ識別子へ吸収
される**（`crates/banto-expr/src/lexer.rs`）。したがって日本語名・空白入りに
加え、`abc-`・`a--b` のような ASCII だけの名前も参照できない。判定は
クライアント側の純関数
...
（`error.kind` の `cycle`/`string_ref`/`unknown_tag`）**と保存時の preflight**
である。
...
**2026-09-06 追記（T21 構成補助 MCP）**: MCP に管理面ツール（接続/グループ/タグ CRUD・
設定・収集/write 制御・API キー発行/失効・`lock_down`）を追加した
（banto-hub-mcp-reference.md §6、banto-hub-t21-design.md）。これらは新スコープ
**`admin`** を要求し、**ロックダウン後も `admin` スコープの API キーで利用できる**
（＝「ロックダウン＝構成凍結」の意図的な緩和。オーナー決定 banto-hub-t21-design.md
§2・§3.2）。write 系データツールは従来どおりロックダウンでアドバイザリ化するが、
構成ツールは **`admin` スコープ・全操作の監査（`origin:"mcp"`）・不可逆操作の confirm**
の3点で守る。admin キーの配布を厳格に管理することが前提。
...
5. **読み書き単一セッション**: 書き込みは収集と同じ PLC セッションを通す
   ...
   **外**で生存させ、構成再構築を跨いで SLMP セッションを維持する
   ...
   存在しないため共有の必要がない）。
   **#131 後半スライス追記（2026-09-01）**: banto-broker のプロトコル
   抽象化（#130）に続き `"modbus-tcp"` ドライバが追加され、Modbus TCP
   接続の**書き込み・ステータス表示**も broker 管理に加わった
   （`banto_broker::is_supported_protocol`）。**収集の読み取りは変えて
   いない** — banto-hub の `hub_client_factory` は Modbus 接続の読み取りを
   引き続き `banto-collect` 自身の直接 `ModbusTcpClient` に委ねる。この結果、
   収集が稼働中の Modbus 接続へ書き込みが起きると、読み取り用（直接
   クライアント）と書き込み用（broker セッション）で**2本の TCP ソケット**
   が同時に張られる — SLMP のように「読み書きを1セッションに統合する
   必要」自体は元々 R08ENCPU の実機ハードウェア制約（同一ポートへの
   2本目接続をタイムアウトさせる、本節冒頭参照）から来ていたもので、
   一般的な Modbus TCP サーバーの多くは複数クライアント接続を許容する
   ため、この2ソケット化は許容済みの意図的なトレードオフとする
   （実機で単一接続しか許さない Modbus デバイスが見つかった場合は
   別途対応を検討する）。
   **2026-09-08 オーナー決定（issue #337、上記の2ソケット化トレードオフを
   撤回）**: 直前の「2ソケット化は許容する」判断を**覆し、Modbus TCP の
   収集読み取りも broker セッションへ相乗りさせる**（= SLMP と同じく
   **1接続 = 1ソケット**）。撤回の理由は、上で「見つかった場合は別途対応」
   としていた**単一接続しか許さない Modbus デバイスが実在した**こと:
   オムロン KM-D1-ETN 電力量モニタ（マニュアル KANC-718B 12-1）は1対1接続
   しか受け付けず、実機では broker セッションが先に接続して12秒間無通信で
   居座り、収集用の2本目が機器側から即切断され、以後1秒ごとに再接続→即切断
   を繰り返して全タグが Bad のままになった（TCP プロキシで観測）。
   「多くの Modbus TCP サーバーは同時接続を許容する」という #131 当時の
   前提は、許容する機器に対しては正しかったが、許容しない機器では
   **収集が一切成立しない機能不全**になる。相乗り自体に新規機構は不要で、
   `banto-broker` の `ModbusSession` は `BrokerSession::read_batch` を
   直接クライアントと同じ `plan_batch_requests` + `execute_modbus_reads`
   で実装済みのため、コイル/ディスクリート入力/ワード内ビット/32bit/64bit
   のすべてが等価に動く。トレードオフとして、**収集読み取りと書き込みが
   1本のソケット上で直列化**され、長い書き込みが収集ティックを遅らせうるが、
...
6. **書き込み受付の既定と永続（2026-09-09 オーナー決定で撤回・変更、#340）**:
   ...
   hub はルールエンジンを持たないパススルーで、外部の認証済みクライアント
   の明示要求を1回転送するだけであり、自律再開という危険が無い。書き込み
   可否の実体は per-tag `writable`（既定 false）と API キーの `write`
   スコープが担う。旧ルール「起動時は必ず disabled とし、管理 UI で明示的
   に有効化する（relay-wright のアーミングと同じ『再起動で必ず安全側』）」
...
7. **プロトコル整合（2026-08-05 オーナー承認、2026-09-01 #131 で解消）**:
   ...
   対応）を T2 の前提スライスとし、書き込み第一弾は SLMP** とする。Modbus
   書き込み（banto-plc-write への FC5/6/15/16 追加 + broker のプロトコル
   抽象化）は**I9 バックログ**だったが、#130（プロトコル抽象化）+ #131
   （Modbus TCP ドライバ）で完了した。受付経路: Modbus 接続配下のタグも
   Numeric（数値/ビット、FC5/6/15/16）は書き込み可能になった。String と
   BitInWord（ビット単体）は引き続きスコープ外で、個別リクエストが
   `Bad` になる（§6.2 参照）
...
- **bit 位置の基数はプロトコルごとに異なる（T20-④、2026-09-04 オーナー
  決定）**: **SLMP は16進1桁 `.0`〜`.F`**（`D100.F` は bit 15）。実際の
  MELSEC engineering tool（GX Works2/3 等）がワードデバイスのビット位置を
  常に16進で表記する慣習に合わせたもの。T8 実装当初は「MELSEC はビット
  位置を10進で書く」という誤った前提で10進1〜2桁（`.0`〜`.15`）を実装
  していたが、これは事実誤認であり本決定で是正した（`D100.10`〜
  `D100.15` のような旧・10進2桁表記は今後拒否される — 移行が必要な既存
  タグがあれば16進表記へ書き換える）。**Modbus は従来どおり10進のまま**
  （`40001.3` — レジスタ内ビット位置を10進で書く Modbus ツールの慣習に
  合わせている）。両者は単に慣習が異なるだけで、桁上がりや読み書きの
  挙動そのものに違いはない。実装は `crates/banto-plc/src/slmp/address.rs`
  （SLMP、`parse`/`format`）と `crates/banto-plc/src/address.rs`
  （Modbus、10進のまま）。
...
### 6.2 Modbus 書き込み: 読み取り専用領域はレジストリで拒否（2026-09-01 オーナー決定A）

Modbus で書き込めるのは Coil（0xxxx）と HoldingRegister（4xxxx）だけである。
DiscreteInput（1xxxx）と InputRegister（3xxxx）は Modbus 仕様上ずっと
読み取り専用で、どんな書き込みスタックを実装しても永久に書けない。
したがって `writable = true` かつ読み取り専用領域を指すタグは、**登録時
（banto-tags, I1）に拒否する**。

これは §6 冒頭〜項目7の「`writable` の protocol 対応可否は banto-tags
（レジストリ）ではなくアプリ層/書き込みスタックの関心事」という既存の
線引きに例外を1つ足すものだが、線引き自体は変えない。理由は2つの事実の
性質が違うから:

- protocol 対応の有無（項目7 - 従来「writable にできるのは SLMP 接続配下の
  タグのみ」だった制約）は #131 が段階的に塞いだ**一時的な能力差**（2026-09-01
  時点で解消済み）で、引き続きアプリ層（`write_path.rs`）の関心事のまま
- 領域のリードオンリー性は**アドレスに内在する恒久的な性質**（DiscreteInput/
  InputRegister はどの実装であっても書けない）で、レジストリが既に持っている
  アドレス検証＝「行として妥当か」の一部に収まる

既存の `writable` な Modbus タグ（読み取り専用領域を含む）は**移行しない**。
現状もアプリ層が Modbus タグへの書き込みを一律拒否しており（項目7）実害が
ないためで、新規作成・更新時の入力だけを検証する。

実装は `crates/banto-tags/src/tag.rs` の `validate_tag_input`
（`modbus_read_only_area` ヘルパー）。`crates/banto-plc-write` の
`modbus::planning::plan_modbus_writes` も同じ理由で**防御的に**同じ判定を
行う（`PlcWriteError::ModbusReadOnlyArea`）- レジストリを経由しない直接の
書き込み要求に対する多層防御。

**#131 前半スライスの状態（2026-09-01）**: `banto-plc-write` に Modbus
書き込み（FC5/6/15/16）のエンコード・実行を実装済み（読み取りプランナと
同じ gap-tolerance-zero 規律、FC16=123レジスタ/FC15=1968コイル上限、
既定 word_order は読み取り側と同じ `HighLow`）。**broker への配線は #130
（PR #214、レビュー中）完了後の次スライス**のため、Modbus タグへの書き込みは
依然としてアプリ層（項目7）で拒否される - この節の決定は「登録できるかどうか」
だけを変え、「書き込みが今使えるかどうか」は変えない。

**#131 後半スライスの状態（2026-09-01 追記）**: broker への配線が完了し、
Modbus タグの Numeric（数値/ビット）書き込みが**実際に使えるようになった**
（上の段落が「次スライス」と呼んでいたものはこれである）。`banto_broker`
に `"modbus-tcp"` ドライバが追加され（#130 のプロトコル抽象化の上に構築）、
`apps/banto-hub`（`crate::write_path::execute_write`）の gate 5（項目7）は
`banto_broker::is_supported_protocol` を見るようになったので、Modbus 接続
配下の writable タグへの書き込み要求はもう明示エラーにならない。String と
BitInWord（ビット単体）は引き続き broker の Modbus ドライバがスコープ外
として扱い、個別リクエストが `Bad(UnsupportedRequestKind)` になる（RMW を
要する BitInWord は Modbus では FC22 Mask Write Register 実装まで対応しない
方針、§6.1 参照）。この節が定義する「読み取り専用領域の登録時拒否」
（`modbus_read_only_area`）は無変更 — 書き込みが実際に通るようになった今、
