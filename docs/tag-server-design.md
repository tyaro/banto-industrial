...
> 調査手段の注記: マニュアルサイト（docs.roboticsware.com）への直接アクセスは
> 2セッションとも不可（コンテナ経路はネットワークポリシーで拒否、
> WebFetch はサイト側ボット対策で 403、web.archive.org 経由も遮断）。
> 本節の内容は**検索エンジンのスニペット経由**で得た二次情報であり、
> 一次情報での裏取りは未了。ページ URL は検索結果に現れた実在のもの。
...
タグパスは Unit / Folder / Tag をピリオド `.` で連結する
（[タグの種類](https://docs.roboticsware.com/jp/6.0.12/fa-server/contents/cmn_tag_type.html)）—
本設計の外部名 `{connection}.{group}.{tag}`（§4）と区切り文字まで一致する。
本設計との対応:

| FA-Server | 本製品群 | 備考 |
| --- | --- | --- |
| Unit / Folder / Tag | `PlcConnection` / `CollectionGroup` / `Tag`（I1） | 3層構造が偶然一致 — 外部名 `{connection}.{group}.{tag}`（§4）は業界慣行と整合（区切りも同じ `.`） |
| スタティックタグ（事前登録・PLC通信） | タグ空間 = `CurrentValuesHandle` | §3.2。FA-Server の既定形態に相当 |
| アクティブタグ（`#` 前置で PLC アドレス直接指定、事前登録なし） | **意図的に非対応** | 本設計はキャッシュ迂回のオンデマンド直接読み出しを提供しない（§5.1 の設計原則）。catalog バインドモデル（§4.1）とも相反する |
| タグ（内部演算ワークエリア） | 演算タグ・内部タグ（§4.2、T6） | サーバー側で一元実装（2026-08-04 決定） |
| エイリアスタグ（別名定義ファイル） | 非対応（安定 ID で代替） | リネーム追従は §4.1 の安定 ID + revision が担う。別名層は導入しない |
| ネットワークタグ（ノード間連携） | 非スコープ | FA-Server 側でも旧方式（下位互換用）で、現行は IPLink サーバーユニットが後継。複数サーバー構成が必要になったら検討 |
| イベント | `CollectEvent` + WS/gRPC/MQTT 配信 | §5 |
| アクション（スクリプト SC1/SC2・メール等） | **アプリ分担で置換**: 自動書き込みは relay-wright、通知は非スコープ | モノリスにしない方針（§2）。§4.2 に文法比較あり |
| ビュー | **アプリ分担で置換**: 記録・トレンドは ChronoGazer、監視画面は将来の SCADA | 同上 |
| インターフェース（OPC DA 1.0/2.0 / DDE / IPLink） | REST / WebSocket / MQTT / gRPC | OPC **UA** を将来枠に（DA/DDE はレガシーのため追わない）。IPLink は同社独自の TCP/IP クライアント/サーバープロトコルで、FA-Panel や VB 製アプリが FA-Server のタグへアクセスする経路 — この構図は §7 の (A) データプレーンクライアントと同型（初版で「Panel IF」と記した経路の正式名） |
...
**FA-Server の演算機構との比較（マニュアル調査 2026-08-04、スニペット由来）**:
FA-Server のサーバー側演算は3系統のスクリプト構文で行う — SC1（VB 互換
演算子によるタグ同士の簡易演算）、SC2（if / for・ユーザー定義関数・文字列
結合・時刻演算まで持つフル言語）、演算式構文。いずれも「アクション」として
...
- 比較材料の注記: FA-Server のクライアント購読経路 IPLink（§1.1）は独自
  バイナリプロトコルで、購読粒度・更新通知の詳細仕様は公開マニュアルの
  スニペットからは得られなかった（2026-08-04 調査）。本節の購読設計は
  比較対象なしの独自設計として確定してよい — Web 標準（WS + JSON）で
  あること自体が IPLink（専用クライアント前提）に対する差別化になる
...
2. **I1 スキーマ拡張**: `writable` フラグ + タグ種別（§4.2: `plc`/`computed`/
   `internal`、`expression`、`retain`）の置き場所（`tags` 列追加 vs
   タグサーバー側の別テーブル）。I1 は3アプリ共有クレートのため、列追加なら
   ChronoGazer / relay-wright のマイグレーション追従が必要。1回の拡張に
   まとめる
...
12. ~~内部タグを作るか~~ → **採用決定**（2026-08-04、§4.2 で演算タグと
    合わせてタグ種別として設計済み）。残る詳細判断:
    - **式文法の確定**（§4.2）: 最小セット（四則・比較・論理・if・
      min/max/abs/round/clamp）で v1 として足りるか。文字列演算・
      ビット抽出（ワードからのビット取り出し）の要否
