...
§5.6 の #332 の記述から relay-wright（凍結）を外した。2026-09-16 更新: #342 段階B のセグメント補完と
...
（Hub 経由ドライバ = #383 段階1）。「relay-wright の読み取り委譲」は relay-wright 自体の凍結に
伴い保留。
...
| 着手    | ChronoGazer の banto-hub 経由ドライバ          | 2026-09-17 決定で v2 候補から格上げ（#383 段階1）。購読（`TagClientHandle`）で値を受け、SLMP / Modbus TCP の直結ドライバと並べる。「収集をタグサーバーへ全面委譲し UI と収集の寿命を分離する」完全な (A) 型モードは引き続き先の話                     |
