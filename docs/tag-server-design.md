...
| Method | Path | 内容 |
| --- | --- | --- |
| GET | `/api/v1/tags` | catalog: 全タグの定義 + メタデータ（`?group=`, `?connection=` フィルタ） |
...
## 7. 既存アプリとの関係・移行ロードマップ

v1 時点では3アプリは独立製品であり、同居は次の段階制で解消していく:

| 段階 | 構成 | 説明 |
| --- | --- | --- |
| 現状 | 各アプリが直接 PLC 接続 | 単独稼働なら問題なし。同居時はセッション数に注意（W5 実機検証の結果待ち） |
| v1 | タグサーバー単独でも製品 | 外部システム連携（MES/クラウド/自作画面）が主用途。ChronoGazer 等との同居は「別 PLC or セッション余裕あり」の現場のみ |
| v2 候補 | ChronoGazer リモート収集モード | ChronoGazer に「収集をタグサーバーへ委譲し、自分は WS/REST + tsquery で読む」モードを追加。UI と収集の寿命分離も同時に達成 |
| v2 候補 | relay-wright の読み取り委譲 | 条件評価の入力読み取りをタグサーバー購読へ。**書き込みセッションは relay-wright 専有を維持**（安全機構と一体のため） |

タグレジストリ DB はアプリ毎に別ファイルのまま（現状と同じ）。同居時の
定義共有は「タグサーバーが正、他アプリはエクスポート/インポート」から始め、
共有 DB 化は必要になってから判断する。
