...
剪定（`AuditLogService::prune`）はサーバー起動時に1回と、
`POST /api/audit-log/list`（監査ログ画面の表示）のたびに軽く
実行されます。専用のバックグラウンドタスクは意図的に持たせていません -
管理者だけがアクセスする画面で、剪定自体もインデックス済みの
`DELETE` 数本なので、この2箇所で十分という判断です
（ChronoGazer/relay-wright と同じ設計判断）。
