...
13. [接続単位のシミュレーションモード](#13-接続単位のシミュレーションモード)
14. [ライブタグモニタ](#14-ライブタグモニタ)
15. [タグの連続登録](#15-タグの連続登録)
16. [タグの CSV インポート/エクスポート](#16-タグの-csv-インポートエクスポート)
17. [PLC 接続テスト](#17-plc-接続テスト)
...
- **Windows サービスの自動登録**: インストール完了時（post-install
  ...
  失敗した場合は §10「サービスの登録（install）」の手順で手動登録して
  ...
  です - §10「起動確認」の手順で別途行ってください。**新規インストールの
...
- **`/api/v1/status` / `/api/v1/tags` での確認**: `GET /api/v1/status`
  ...
  （§9「死活監視」の定期ポーリングに `simulation` の監視も組み込むことを
  推奨します）。
...
- **品質の色分け**: `status/+page.svelte`（§9 死活監視の一覧）と同じ規約
  です — good は通常色、bad は danger（太字）、stale は muted。
- **フィルタ**: 接続・グループのプルダウンでクライアント側のみ絞り込み
  ます（再取得は発生しません）。
- **シミュレーションバッジ**: T9（§13）でシミュレーションモードの接続に
...
- この一括登録 API は T11-2（CSV インポート）とも共有される共通基盤です。
  詳細は[§16「タグの CSV インポート/エクスポート」](#16-タグの-csv-インポートエクスポート)
  を参照してください。

## 16. タグの CSV インポート/エクスポート

T11-2（[ux-plan.md](ux-plan.md) §3）。タグ定義を CSV でバックアップ・
複製・レビューできます。§15 の連続登録と同じ一括登録 API
...
毎に同時セッションを張れる — §13 のシミュレーション経路や broker の設計は、
