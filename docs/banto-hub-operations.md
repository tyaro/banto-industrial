...
### 重要な注意: SLMP のセッション数上限（実機 R08ENCPU で1本の実測あり）

三菱 R08ENCPU 実機は SLMP の同時 TCP セッションを1本しか受け付けません
（§13 のシミュレーション経路や broker の設計もこの制約が前提です）。この
ため、接続テストは**保存済み接続を編集中**の場合、その接続の収集が
...
するしかありません。この場合、対象 PLC が既に別セッション（この hub
自身の収集や、他のアプリケーション）と接続中だと、SLMP のセッション数
上限により接続に失敗することがあります。この状況では失敗メッセージに
その旨の注意が付きます — 「設定自体は正しいのにテストが失敗する」場合、
まず対象 PLC への他の接続（収集中の別セッション、GX Works 等）が無いか
確認してください。
