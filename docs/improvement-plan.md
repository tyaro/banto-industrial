...
| H5  | フロントテスト基盤(vitest)+ hub/relay-wright E2E         | 高     | 大   | 実装済・lock 待ち |
...
### H5: フロントテスト基盤 + E2E 拡充 — 状態: 初段実装済み・lockfile 待ちでブロック(2026-08-08)
...
- **初段の実施状況(2026-08-08)**: `apps/banto-hub` に vitest を導入
  (`package.json` に `vitest` devDep + `"test": "vitest run"`)、
  `vitest.config.ts` と `tagCsv.test.ts`(97 の手書きテスト、`tagCsv.ts`
  の全公開 API を網羅)を実装済み。実 vitest で 121 テスト green・
  svelte-check 0 エラーまで検証済み。**ただし PR #59 には未着地**:
  vitest 追加に伴う `pnpm-lock.yaml` 更新が必須だが、この開発
  サンドボックスのプロキシは lockfile 再生成時に全 `@banto/*` git 依存の
  URL を CI 非互換の形式(`git+https://git@github.com:…` の SSH 風)へ
  書き換えてしまい、安全な lockfile を生成できない。lockfile を正しく
  更新できる環境(GitHub 直アクセス)からの `pnpm install` を待って着地
  させる。実装物はスクラッチパッドに保全済み
