...
作成日: 2026-08-08
状態: **進行中(Phase 1 = PR #58、Phase 2 = PR #59/#73 マージ済み、Phase 3 = H10 ①② を PR #74 で実装)**。
H1〜H4・H6・H8 完了、H5 は vitest 導入まで完了(E2E 拡充は Phase 4)。
H10 は ①任意期限+UI 警告・②arm 時限失効を実装、③read スコープ細分化は
実装前に案を提示してから着手。
...
| H10 | 認可の細粒度化(キー有効期限・read スコープ・arm 失効)    | 中     | 中   | ①② 実装(#74)・③案待ち |
...
### H10: 認可の細粒度化 — 状態: ①② 実装済み(PR #74、2026-08-08)・③ は案提示後
...
- **実施記録(2026-08-08、PR #74)**: ①② を実装(③ は対象外)。
  ...
  - **③ read スコープのタグ単位化**は本 PR に含めない — catalog API の扱い
    (スコープ外タグを一覧から隠すか、値だけ読めなくするか)の比較案を
    オーナーへ提示してから着手(上記決定どおり)
...
- **Phase 3(PR #74、2026-08-08 実装)**: H10 ①任意期限+警告・②arm 時限
  失効(③read スコープ細分化は案の提示から、本 PR 対象外)
