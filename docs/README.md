...
最終更新: 2026-09-06（T19（UX-30〜48）・T20（文字列/構造体/レシピ/ビット .0〜.F）・T21（構成補助 MCP 管理面）完了、MCP 31 ツール実機検証、外部 DB 連携 S0〜S5（MCP は 37 ツールに。追加分はローカル PostgreSQL で検証）を反映）。
...
| [banto-hub-external-db-design.md](banto-hub-external-db-design.md) | **外部 DB 連携（#228 Source / #229 Sink）の設計**。S0〜S5 完了、S3/S6/S7 残。DB 接続を Source/Sink で共有し、Source は既存の接続→グループ→タグ 3 階層を流用（案 A）して Hub 内で動き、Sink は設定と監視を Hub に置いた別プロセスのサイドカー `apps/banto-hub-sink`。 |
