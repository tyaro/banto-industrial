...
最終更新: 2026-09-06（T19（UX-30〜48）・T20（文字列/構造体/レシピ/ビット .0〜.F）・T21（構成補助 MCP 管理面）完了、外部 DB 連携 S4（Sink グループ管理ツール 5 個追加）、MCP 37 ツール実機検証を反映）。
...
- **MCP（機械/AI 向け外部 IF）**: `POST /mcp`（API キー認証）で **37 ツール** — データ面（値の
  ...
  収集/write 制御・API キー発行/失効・lock_down・Sink グループ管理）。実機 R08ENCPU で全ツール検証済み
...
| [banto-hub-external-db-design.md](banto-hub-external-db-design.md) | **外部 DB 連携（#228 Source / #229 Sink）の設計**。S0〜S5 完了、S3/S6/S7 残。DB 接続を Source/Sink で共有し、Source は既存の接続→グループ→タグ 3 階層を流用（案 A）、Sink は MQTT 発行と同型の読み取り専用消費者。 |
