...
recorder-requirements.md §3.1）。以下は経緯。2026-09-21: #383 段階2b / R1-C: chronogazer に収集ランタイムの
...
- **ChronoGazer の3ドライバ構成（#383、2026-09-17 オーナー決定）— 段階1 完了、段階2・3 は未着手**:
  ...
  **段階2a（= r1-plan.md R1-B、レジストリ CRUD）は実装済み（2026-09-18）**:
  ...
  R1-C、収集ランタイム統合）は C-1〜C-4 に分割して着手中**: C-1（収集サービスの
  ...
  イベント一覧。現在値の表示は R1-D の監視画面、#409）が入った（2026-09-21）。
  **一巡はまだ通っていない** -
  シミュレータ相手の「設定 → 収集開始 → データファイル生成 → イベント記録」
  （C-4）が残りで、r1-plan.md R1-C の完了条件を満たすのはそこから。収集エンジンは
...
| [r1-plan.md](r1-plan.md)                                     | 記録計 R1 実施計画。**運用中**（R1-A 完了、R1-B は #383 段階2a で着手・レジストリ CRUD 実装済み・接続単位シミュレーションを #413 で開けた（2026-09-23 オーナー決定）、R1-C は C-1/C-2/C-3a/C-3b 実装済み・C-4 残り、R1-D 未着手）。 |
