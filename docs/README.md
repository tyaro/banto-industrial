...
| [banto-hub-external-db-design.md](banto-hub-external-db-design.md) | **外部 DB 連携（#228 Source / #229 Sink）の設計、Draft・オーナー決定待ち**。DB 接続を Source/Sink で共有し、Source は既存の接続→グループ→タグ 3 階層を流用（案 A）、Sink は MQTT 発行と同型の読み取り専用消費者。オーナー決定項目は §6、スライス案は §7。 |
