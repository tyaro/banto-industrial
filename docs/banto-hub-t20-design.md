...
**2026-09-08 追記（issue #325）**: `data_type` に 64bit 型 `i64`/`u64`/`f64` を追加（詳細・配置制約は [tag-server-design.md](tag-server-design.md) §6.3 が正）。②構造体タグ登録のワードサイズ考慮の連番割付に `i64`/`u64`/`f64`=4 を追加（§3.2）。
...
- **構造体の一括登録**: 複数フィールド（名前・型・任意の相対オフセット）をまとめて登録する
  ...
  i64/u64/f64=4〈2026-09-08、issue #325。Modbus 接続配下のみ〉、
