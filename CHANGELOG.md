...
banto v1.7.1 に追従し、ユーザーの削除・降格・パスワード変更・パスワードリセットで、そのアカウントの既存のセッションが終わるようにした（banto #204、v1.7.0 で導入）。配布物の構成・前提ランタイムは alpha.3 以降と同じ。**wire 変更なし**（ただし、下記のとおり照合できないときの応答に `500` が増えた。#433 で書き込み受付の応答と状態に注意書きのフィールドを足した）。
...
- **API キーで開いた WebSocket（`/api/v1/stream`）が、キーを失効・トリップしても、期限が切れても、切断まで値の配信を受け取り続けていた**（#430、認証は接続時だけだった）。開いている間も 15 秒ごとに `ApiKeysService::check` → `api_key_verdict`（#434 / #435 と同じ分類）で照合し直し、使えないと確認できたら close code `1008` と理由文（`api_key_revoked` / `api_key_expired` / `api_key_tripped` / `api_key_not_found`）で閉じる。照合できない（DB エラー・5 秒のタイムアウト）ときは閉じない。次の照合は照合が終わった時点から 1 周期後で、照合は配信と並行に進む（照合を待つ間も値の配信は止まらない）。タイマーはストリームごとに 1 つで、切断で照合中のものごと捨てる。セッションで開いたストリーム（`/api/tag-stream` と `/api/v1/stream` のセッション）は、banto の `AuthState::revalidate` の公開（tyaro/banto#239）を待って同じ仕組みに足す（今は従来どおり接続時の照合だけ）。

### 変更（内部、2026-09-24）

- banto の依存（`banto-core` / `banto-storage` / `banto-server`、`@banto/*`）を `v1.7.1` に上げた（2026-09-25。セッション失効の banto #204 は v1.7.0、グリッドで画面を開いた直後の行クリックを取りこぼす不具合の修正 banto #236 と、アイドルのタイマーを延ばさずにセッションを再検証する `AuthState::revalidate` の公開 banto #239 は v1.7.1）。未保存の変更の確認（banto #214、`@banto/forms` の `guardUnsavedChanges`）の画面への適用は次の PR で行う。
