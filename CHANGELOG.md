...
- **非常停止（書き込み受付の無効化）を DB に保存できなかったとき、再起動で書き込み受付が有効に戻っていた**（#340 からの挙動）。状態を DB と、データディレクトリの状態ファイル `write-control-state.json`（一時ファイルに書いて `rename` する原子的な書き込み）の 2 か所に記録し、**起動時はどちらか一方でも無効（または読めない・壊れている）なら無効で起動する**ようにした（状態ファイルが無い既存環境は DB に従う）。無効化はどちらか一方に保存できれば成功（`200`、DB が応答しないときも状態ファイルに書ければ成功）とし、保存できなかった側を応答の `persistence_warning`・ログ・監査の `detail`（`persistedDb` / `persistedFile` / `dbError` / `fileError`）に出す。両方に失敗したときだけ従来どおり `500 write_control_persist_failed`。有効化は両方に保存できたときだけ受付を再開し、保存中に無効化が割り込んだら無効化を優先する（`409 write_control_stop_interrupted`）。起動時の食い違いと直近の保存の失敗は、起動ログと管理 UI の状態画面（`GET /api/v1/status` の `write_persistence_warning`）に出す。MCP の `set_write_control` も同じ処理を通る。
