...
banto v1.7.1 に追従し、ユーザーの削除・降格・パスワード変更・パスワードリセットで、そのアカウントの既存のセッションが終わるようにした（banto #204、v1.7.0 で導入）。配布物の構成・前提ランタイムは alpha.3 以降と同じ。**wire 変更なし**（ただし、下記のとおり照合できないときの応答に `500` が増えた）。
