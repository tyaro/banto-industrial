...
banto v1.7.1 に追従し、ユーザーの削除・降格・パスワード変更・パスワードリセットで、そのアカウントの既存のセッションが終わるようにした（banto #204、v1.7.0 で導入）。配布物の構成・前提ランタイムは alpha.3 以降と同じ。
...
- 開いている WebSocket のストリーム（`/api/tag-stream`・`/api/v1/stream`）は、セッションや API キーが失効・期限切れ・トリップしたと確認できたら、close コード `1008` と理由文（`session_revoked` / `api_key_revoked` / `api_key_expired` / `api_key_tripped` / `api_key_not_found`）で閉じる（#430）。
...
- **ストリームが切れている間にセッションが失効すると、管理 UI のタグモニタが再接続を繰り返していた**（#441 の残り）。再接続のハンドシェイクは認証で拒否されるが、ブラウザの WebSocket からは理由が見えず `1006` にしかならないため、通常の切断と区別できなかった。開く前に閉じた再接続を数え、**2 回続けて失敗したら、画面を動かさずにログイン状態を確かめる**（ルートガードと同じ部品: 試運転モードの状態 → `decideProtectedRoute`）。失効を確認できたら再接続をやめ、`1008` + `session_revoked` と同じ経路（ルートガードを走らせ直して `/login` へ）に合流する。まだ有効なら再接続を続けて数え直す。照合できない（ネットワークの切断が続いている・`500`・10 秒で返らない）なら、エラー画面へは移らずにバックオフを守って再接続を続ける（確認は再接続 1 回につき高々 1 回）。接続が開いたら数え直し、確認の途中で接続が開いた・画面を離れたときは、返ってきた結果を捨てる。判断は純関数 `shouldProbeSession` / `decideAfterSessionProbe`（`streamClose.ts`）。失効に気づくまでは、切断の直後なら約 3 秒、最悪でも失効から再接続 2 回ぶん（最大 60 秒）+ 確認 1 回。
