...
test.describe.serial('banto-hub 狭幅でツリーペインを退避する (#378)', () => {
	...
	test('5. モーダルの外にフォーカスがある状態の Esc でも、閉じるのはモーダルだけ（#381 レビュー対応2回目）', async () => {
		// 通常の Tab では外へ出ない（この PR で共有のフォーカストラップを入れた -
		// `focusTrap.ts`）が、ここでは `blur()` で**意図的に**フォーカスをモーダルの
		// 外へ出し、その状態の Esc を見る（外部要因でフォーカスが外れることは
		// ありえるので、window レベルのフォールバックが要る）。退避パネルの
		// フォールバックが**発生元**だけを見ていると、この状態の Esc で
		// 「モーダルと退避パネルが両方閉じる」ことになる。
		//
	...
	test('14. 接続 Drawer が開いている間はグループ Drawer を開かない（未保存入力を捨てない、#381 レビュー対応8回目）', async () => {
		...
		// モーダルのオーバーレイがツリーを覆うのでクリックでは届かない - ボタンへ
		// 直接 click イベントを送って再現する。共有のフォーカストラップ
		// （`focusTrap.ts`）が入った今、Tab でここへ到達する経路は塞がっているが、
		// **プログラム的な起動（トラップをすり抜ける経路）**でも同時に2つ開かない
		// ことを排他ガードで担保する、という確認。
